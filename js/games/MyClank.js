...
export default {
  ...
  init: (container) => {
    ...
    //Erstellen der random givens
    const CIRCLE = 1;
    const SQUARE = 2;
    const Bsize = 6;
    let Tgivens = Array.from({ length: Bsize }, () => Array(Bsize).fill(0));

    const isValid = (row, col, value) => {
      // Überprüfen der Zeile
      if (
        Tgivens[row].slice(Math.max(0, col - 2), col).filter((v) => v === value)
          .length >= 2
      )
        return false;
      ...
      if (
        Tgivens.slice(Math.max(0, row - 2), row).filter((r) => r[col] === value)
          .length >= 2
      )
        return false;
    ...
    const fillBoard = () => {
      for (let row = 0; row < Bsize; row++) {
        ...
        for (let col = 0; col < Bsize; col++) {
          let value;
          do {
            value = Math.random() < 0.5 ? CIRCLE : SQUARE;
          } while (
            (value === CIRCLE && circleCount >= 3) ||
            (value === SQUARE && squareCount >= 3) ||
            !isValid(row, col, value)
          );

          Tgivens[row][col] = value;
          if (value === CIRCLE) circleCount++;
          else squareCount++;
        }
      }
      console.log(Tgivens);
