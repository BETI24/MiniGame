const RISK_PROFILES = {
  low: {
    ...
    multipliers: {
      ...
      14: [
        10.0, 3.4, 2.0, 1.4, 1.15, 1.0, 0.9, 0.8, 0.9, 1.0, 1.15, 1.4, 2.0, 3.4,
        10.0,
      ],
      16: [
        12.0, 4.0, 2.4, 1.6, 1.25, 1.05, 0.95, 0.85, 0.75, 0.85, 0.95, 1.05,
        1.25, 1.6, 2.4, 4.0, 12.0,
      ],
    },
  },
  medium: {
    ...
    multipliers: {
      ...
      14: [
        42, 10, 4.2, 2.2, 1.35, 0.9, 0.55, 0.3, 0.55, 0.9, 1.35, 2.2, 4.2, 10,
        42,
      ],
      16: [
        60, 14, 6.0, 3.0, 1.8, 1.15, 0.75, 0.45, 0.25, 0.45, 0.75, 1.15, 1.8,
        3.0, 6.0, 14, 60,
      ],
    },
  },
  high: {
    ...
    multipliers: {
      ...
      14: [
        180, 38, 12, 4.2, 1.4, 0.5, 0.2, 0.06, 0.2, 0.5, 1.4, 4.2, 12, 38, 180,
      ],
      16: [
        260, 55, 18, 6.5, 2.2, 0.8, 0.3, 0.12, 0.04, 0.12, 0.3, 0.8, 2.2, 6.5,
        18, 55, 260,
      ],
    },
  },
  ...
  init: (container, services) => {
            ...
            <div class="pl-controls">
                ...
                <div>
                    ...
                    <div class="pl-segment pl-risk-buttons">
                        ${Object.entries(RISK_PROFILES)
                          .map(
                            ([key, value]) => `
                            <button class="pl-seg-btn ${
                              key === riskKey ? "selected" : ""
                            }" data-risk="${key}" type="button">
                                ${value.label}
                            </button>
                        `
                          )
                ...
                <div>
                    ...
                    <div class="pl-segment ball-count pl-ball-count-buttons">
                        ${BALLS_PER_DROP_OPTIONS.map(
                          (value) => `
                            <button class="pl-seg-btn ${
                              value === ballsPerDrop ? "selected" : ""
                            }" data-ball-count="${value}" type="button">
                                ×${value}
                            </button>
                        `
                        ).join("")}
    ...
    const rtpAdjustedMultipliers = (profileKey, rowCount) => {
      ...
      const rawRtp = raw.reduce(
        (sum, multiplier, index) => sum + multiplier * probabilities[index],
        0
      );
    ...
    const currentMultipliers = () => rtpAdjustedMultipliers(riskKey, rows);
    ...
    const updateHud = () => {
      ...
      dropBtn.disabled = credits < bet || balls.length >= CONFIG.maxBalls;

      riskLabelEl.textContent = `${currentProfile().label} · RTP ${(
        currentTheoreticalRtp() * 100
      ).toFixed(2)}%`;
    ...
    const setBet = (value) => {
      const maxAllowed = Math.min(
        ...
        Math.max(
          CONFIG.minBet,
          Math.floor(Math.max(credits, CONFIG.minBet) / CONFIG.betStep) *
            CONFIG.betStep
        )
      );
    ...
    const buildBoard = (width, height) => {
      ...
      board.pegGapX = Math.min(boardWidth / (rows + 2.8), board.pegGapY * 1.15);
    ...
    const createDropOutcome = () => {
      ...
      let rightsBefore = 0;
      const contactOffset = (CONFIG.ballRadius + CONFIG.pegRadius) * 0.78;
      ...
      for (let row = 0; row < rows; row++) {
        ...
        const pegX = board.centerX + (rightsBefore - row / 2) * board.pegGapX;

        const pegY = board.top + (row + 1) * board.pegGapY;
    ...
    const spawnParticles = (x, y, color, count = 12) => {
      for (let i = 0; i < count; i++) {
        ...
        effects[effects.length - 1].maxLife = effects[effects.length - 1].life;
      }
    };
    ...
    const addResultFeed = (multiplier, payout, stake) => {
      ...
      card.className = `pl-result-card ${net >= 0 ? "win" : "loss"}`;

      card.innerHTML = `
                <div class="pl-result-top">
                    <span class="pl-result-multi">${formatMultiplier(
                      multiplier
                    )}×</span>
                    <span class="pl-result-value">${
                      net >= 0 ? "+" : ""
                    }${formatNumber(net)}</span>
                </div>
                <div class="pl-result-sub">${formatNumber(
                  stake
                )} → ${formatNumber(payout)}</div>
            `;
    ...
    const resolveBall = (ball) => {
      ...
      services?.highscores?.saveHighscore?.("plinko", Math.round(peakCredits));
    };
    ...
    const createBallForDrop = (stake, spawnDelay = 0) => {
      ...
      const segmentDuration = clamp(0.148 - rows * 0.0015, 0.12, 0.136);
    ...
    const dropBall = () => {
      ...
      for (let i = 0; i < batchCount; i++) {
        ...
        const spawnDelay = batchCount === 1 ? 0 : i * 0.045;

        balls.push(createBallForDrop(bet, spawnDelay));
      }
    ...
    const updateBalls = (delta) => {
      ...
      for (const ball of balls) {
        ...
        const from = ball.waypoints[ball.segmentIndex];

        const to = ball.waypoints[ball.segmentIndex + 1];
        ...
        const t = clamp(ball.segmentElapsed / ball.segmentDuration, 0, 1);
        ...
        const smoothX = t * t * (3 - 2 * t);

        const gravityT = 0.28 * t + 0.72 * t * t;

        const direction =
          to.direction ??
          ball.directions[
            Math.min(ball.segmentIndex, ball.directions.length - 1)
          ] ??
          1;

        const arc =
          Math.sin(Math.PI * t) * Math.min(4.5, board.pegGapY * 0.075);
        ...
        ball.y = from.y + (to.y - from.y) * gravityT - arc;
        ...
        if (to.peg) {
          const pegColor = to.direction > 0 ? "#89f2ff" : "#c4e9ff";

          effects.push({
            type: "ring",
            x:
              to.x -
              to.direction * (CONFIG.ballRadius + CONFIG.pegRadius) * 0.78,
            y: to.y + 1,
            radius: 2,
            maxRadius: 9,
            life: 0.18,
    ...
    const updateAuto = (delta) => {
      ...
      if (autoRemaining <= 0 || credits < bet * ballsPerDrop) {
        autoMode = false;
        autoRemaining = 0;
        return;
      }
      ...
      if (autoTimer <= 0) {
        ...
        if (dropped) {
          autoRemaining--;
          autoTimer =
            ballsPerDrop >= 5 ? 0.28 : ballsPerDrop >= 3 ? 0.23 : 0.18;
        } else {
          autoTimer = 0.08;
        }
      }
    ...
    const drawBoardGuide = () => {
      ...
      ctx.lineTo(
        board.centerX - halfBottomWidth,
        bottomPegY + board.pegGapY * 0.48
      );
      ctx.moveTo(board.centerX, board.top + board.pegGapY * 0.52);
      ctx.lineTo(
        board.centerX + halfBottomWidth,
    ...
    const drawPegs = () => {
      for (const peg of board.pegs) {
        ...
        const glowStrength = 0.18 + (peg.row / Math.max(1, rows - 1)) * 0.12;
    ...
    const drawSlots = () => {
      for (const slot of board.slots) {
        ...
        drawRoundedRect(x, y, slot.width, board.slotHeight, 8);
        ctx.fill();

        ctx.globalAlpha = 0.82;
        ctx.fillStyle = color;

        drawRoundedRect(x + 2, y + 2, slot.width - 4, 6, 4);
        ctx.fill();
        ...
        const fontSize = Math.max(8, Math.min(12, slot.width * 0.25));

        ctx.font = `900 ${fontSize}px system-ui, sans-serif`;
        ctx.fillText(label, slot.x, y + board.slotHeight * 0.55);
    ...
    const updateRiskSelection = () => {
      riskButtons.forEach((button) => {
        button.classList.toggle("selected", button.dataset.risk === riskKey);
      });
...
export { RISK_PROFILES, ROW_OPTIONS, BALLS_PER_DROP_OPTIONS, CONFIG };
