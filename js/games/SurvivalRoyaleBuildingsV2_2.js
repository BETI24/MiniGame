...
// Hand-authored navigation points. They deliberately sit on both sides of
// every interior doorway, so bots never have to guess their way through walls.
const BASE_NAV_POINTS = [
  {key:'entryCore',x:180,y:238,type:'house-entry'},
  {key:'living',x:180,y:194,type:'house-room'},
  {key:'lowerLeft',x:108,y:205,type:'house-room'},
  {key:'leftDoorSouth',x:121,y:165,type:'house-door'},
  {key:'leftDoorNorth',x:121,y:125,type:'house-door'},
  {key:'upperLeft',x:121,y:96,type:'house-room'},
  {key:'rightDoorWest',x:210,y:166,type:'house-door'},
  {key:'rightDoorEast',x:255,y:166,type:'house-door'},
  {key:'rightMid',x:291,y:166,type:'house-room'},
  {key:'rightTop',x:291,y:93,type:'house-room'},
  {key:'rightBottom',x:291,y:225,type:'house-room'}
];
...
export function houseNavigation(h){
  if(!h)return[];

  const e=houseEntrance(h);
  const points=[
    {
      id:`house-${h.id}-outside`,
      houseId:h.id,
      x:e.outsideX,
      y:e.outsideY,
      type:'house-outside'
  ...
  for(const base of BASE_NAV_POINTS){
    const q=rotatePoint(base.x,base.y,h.rotation??0);

    points.push({
      id:`house-${h.id}-${base.key}`,
      houseId:h.id,
      x:h.x+q.x,
      y:h.y+q.y,
      type:base.type
