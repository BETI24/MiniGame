...
export default {
  ...
  init:(container,services)=>{
    ...
    style.textContent=`
      .cc{position:relative;width:100%;height:100%;min-height:700px;overflow:hidden;background:#07101a;color:#eef6fb;font-family:Inter,system-ui,sans-serif;user-select:none}
      ...
      .cc-overlay{position:absolute;z-index:30;inset:0;display:flex;align-items:center;justify-content:center;padding:20px;background:#02060cca;backdrop-filter:blur(10px);overflow:auto}.cc-overlay.hide{display:none}.cc-menu{width:min(1080px,100%);max-height:calc(100vh - 35px);overflow:auto;padding:25px;border-radius:18px;background:linear-gradient(180deg,#152436,#09121d);border:1px solid #6de0ff2c;box-shadow:0 30px 100px #0009}
      ...
      @media(max-width:900px){.cc-bottom{grid-template-columns:180px 1fr}.cc-shop-layout{grid-template-columns:1fr}.cc-trees{grid-template-columns:1fr 1fr}.cc-info{display:none}.cc-arenas,.cc-setting-grid{grid-template-columns:1fr 1fr}.cc-controls{display:none}.cc-rogue-loop{grid-template-columns:1fr 1fr}}
      @media(max-width:600px){.cc-opts,.cc-trees{grid-template-columns:1fr}.cc-setting-grid{grid-template-columns:1fr 1fr}.cc-bottom{grid-template-columns:150px 1fr}.cc-weapon{flex-basis:88px}.cc-menu{padding:17px}}
    `;
    ...
    function resize(){const r=root.getBoundingClientRect();W=Math.max(640,r.width);H=Math.max(520,r.height);dpr=Math.min(2,devicePixelRatio||1);canvas.width=Math.round(W*dpr);canvas.height=Math.round(H*dpr);canvas.style.width=r.width+"px";canvas.style.height=r.height+"px";ctx.setTransform(dpr,0,0,dpr,0,0);}
