...
const RESCUE_CLEAR_BONUS_MULTIPLIER = 0.8;
...
const DIFFICULTY_PRESETS = {
  easy: {
    label: "Easy",
    description: "Mehr Zeit · offenere Boards · viele starke Kombos",
    timeMultiplier: 1.16,
    mistakePenalty: 3,
    fillMultiplier: 0.95,
    targetMoveRatio: 0.5,
    targetMultiRatio: 0.14,
    targetStrongRatio: 0.2,
    pairWeight: 42,
    tripleWeight: 25,
    quadWeight: 11,
    doublePairWeight: 22,
  ...
  normal: {
    label: "Normal",
    description: "Klassische Regeln · ausgewogener Start",
    timeMultiplier: 1.0,
    mistakePenalty: 3,
    fillMultiplier: 1.0,
    targetMoveRatio: 0.28,
    targetMultiRatio: 0.09,
    targetStrongRatio: 0.13,
    pairWeight: 60,
    tripleWeight: 18,
    quadWeight: 7,
    doublePairWeight: 15,
  ...
  hard: {
    label: "Hard",
    description: "Dichtere Boards · weniger offensichtliche Züge",
    timeMultiplier: 0.93,
    mistakePenalty: 4,
    fillMultiplier: 1.07,
    targetMoveRatio: 0.19,
    targetMultiRatio: 0.045,
    targetStrongRatio: 0.06,
    pairWeight: 76,
    tripleWeight: 12,
    quadWeight: 3,
    doublePairWeight: 9,
  ...
  expert: {
    label: "Expert",
    description: "Sehr dichte Boards · knapper Timer · wenige Gratis-Kombos",
    timeMultiplier: 0.84,
    mistakePenalty: 5,
    fillMultiplier: 1.14,
    targetMoveRatio: 0.11,
    targetMultiRatio: 0.018,
    targetStrongRatio: 0.026,
    pairWeight: 86,
    tripleWeight: 8,
    quadWeight: 1,
    doublePairWeight: 5,
...
const TILE_COLORS = [
  {
    ...
    symbol: "●",
    pattern:
      "repeating-linear-gradient(135deg, transparent 0 4px, rgba(5,12,22,.28) 4px 6px)",
  },
  {
    name: "Blue",
    value: "#2f78ff",
    symbol: "◆",
    pattern:
      "radial-gradient(circle, rgba(255,255,255,.34) 0 1.2px, transparent 1.4px)",
  },
  {
    name: "Green",
    value: "#28d95a",
    symbol: "▲",
    pattern:
      "repeating-linear-gradient(90deg, transparent 0 5px, rgba(5,12,22,.28) 5px 7px)",
  },
  {
    name: "Orange",
    value: "#ff9c2a",
    symbol: "✚",
    pattern:
      "repeating-linear-gradient(0deg, transparent 0 5px, rgba(5,12,22,.26) 5px 7px)",
  },
  {
    name: "Purple",
    value: "#ba5ae8",
    symbol: "✦",
    pattern:
      "repeating-linear-gradient(45deg, transparent 0 5px, rgba(5,12,22,.24) 5px 7px), repeating-linear-gradient(-45deg, transparent 0 5px, rgba(255,255,255,.12) 5px 7px)",
  },
  {
    name: "Pink",
    value: "#ff69dc",
  ...
  },
  {
    ...
    symbol: "■",
    pattern:
      "repeating-linear-gradient(45deg, rgba(5,12,22,.22) 0 3px, transparent 3px 8px)",
  },
  {
    name: "Gold",
    value: "#d7d563",
    symbol: "○",
    pattern:
      "repeating-linear-gradient(0deg, transparent 0 6px, rgba(5,12,22,.23) 6px 7px), repeating-linear-gradient(90deg, transparent 0 6px, rgba(5,12,22,.23) 6px 7px)",
  },
];
...
export default {
  ...
  init: (container, services) => {
    ...
    style.textContent = `
            ...
            .bb-board-area.combo-hit::after {
                animation:bbComboGlowPulse .28s ease-out;
            }
            @keyframes bbComboGlowPulse {
                0% {
                    box-shadow:
                        inset 0 0 52px rgba(var(--combo-rgb),calc(var(--combo-opacity) * .34)),
                        0 0 24px rgba(var(--combo-rgb),calc(var(--combo-opacity) * .18));
                }
                45% {
                    box-shadow:
                        inset 0 0 66px rgba(var(--combo-rgb),calc(var(--combo-opacity) * .42)),
                        0 0 24px rgba(var(--combo-rgb),calc(var(--combo-opacity) * .22));
                }
                100% {
                    box-shadow:
                        inset 0 0 52px rgba(var(--combo-rgb),calc(var(--combo-opacity) * .34)),
                ...
                <div class="bb-menu-card">
                    ...
                    <div class="bb-size-grid">
                        ${Object.entries(BOARD_PRESETS)
                          .map(
                            ([key, p]) => `
                            <button class="bb-size-card ${
                              key === selectedPresetKey ? "selected" : ""
                            }" data-size="${key}" type="button">
                                <b>${p.label}</b><span>${p.rows} × ${
                              p.cols
                            }</span>
                            </button>
                        `
                    ...
                    <div class="bb-difficulty-grid">
                        ${Object.entries(DIFFICULTY_PRESETS)
                          .map(
                            ([key, d]) => `
                            <button class="bb-difficulty-card ${
                              key === selectedDifficultyKey ? "selected" : ""
                            }" data-difficulty="${key}" type="button">
    ...
    const findValidMoves = (source, rows, cols) => {
      ...
      for (let i = 0; i < source.length; i++) {
        if (source[i] === null && evaluateMove(i, source, rows, cols).valid) {
          moves.push(i);
        }
      }
    ...
    const pickBalancedColor = (
      ...
      if (!options.length) {
        options = counts.map((count, color) => ({
          color,
          count,
          jitter: Math.random() * 1.4,
        }));
      }
    ...
    const choosePairDirections = (available) => {
      const oppositePairs = [
        [0, 1],
        [2, 3],
      ].filter((pair) => pair.every((dir) => available.includes(dir)));
      const preferOppositeChance =
      ...
      for (const pivot of shuffle(emptyPivots).slice(0, 70)) {
        ...
        if (canUseSize(2)) {
          patterns.push({
            type: "pair",
            size: 2,
            weight: currentDifficulty.pairWeight,
          });
        }
        if (available.length >= 3 && canUseSize(3))
          patterns.push({
            type: "triple",
            size: 3,
            weight: currentDifficulty.tripleWeight,
          });
        if (available.length >= 4 && canUseSize(4)) {
          patterns.push({
            type: "quad",
            size: 4,
            weight: currentDifficulty.quadWeight,
          });
          patterns.push({
            type: "doublePair",
            size: 4,
            weight: currentDifficulty.doublePairWeight,
          });
        }
        ...
        if (chosenPattern.type === "doublePair") {
          ...
          const verticalTargets = [
            pickRayTarget(rays[0]),
            pickRayTarget(rays[1]),
          ];
          const horizontalTargets = [
            pickRayTarget(rays[2]),
            pickRayTarget(rays[3]),
          ];
          if (
            [...verticalTargets, ...horizontalTargets].some(
              (value) => value === null
            )
          )
            continue;
          ...
          horizontalTargets.forEach(
            (index) => (candidate[index] = secondColor)
          );
    ...
    const verifyGeneratedSolution = (source, solution, preset) => {
      ...
      for (let i = solution.length - 1; i >= 0; i--) {
        ...
        if (
          !result.valid ||
          !sameIndexSet(result.removeIndices, step.removeIndices)
        ) {
          return false;
        }
          ...
          ? Math.max(
              ...
              Math.round(
                total *
                  Math.min(
                    0.82,
                    preset.fillRatio * currentDifficulty.fillMultiplier
                  )
              )
            )
    ...
    const analyzeGeneratedBoard = (candidate, preset) => {
      ...
      const emptyCount =
        candidate.length - candidate.filter((value) => value !== null).length;
      ...
      for (const move of moves) {
        ...
        if (result.groups.some((group) => group.indices.length >= 3))
          strongMatches++;
      }
      ...
      const colorAverage =
        usedColors.reduce((sum, value) => sum + value, 0) /
        Math.max(1, usedColors.length);
      const colorSpread = usedColors.length
        ? 1 -
          Math.min(
            1,
            (Math.max(...usedColors) - Math.min(...usedColors)) /
              Math.max(1, colorAverage)
          )
        : 0;
      ...
      const quadrantAverage =
        quadrants.reduce((sum, value) => sum + value, 0) / 4;
      const quadrantSpread =
        1 -
        Math.min(
          1,
          (Math.max(...quadrants) - Math.min(...quadrants)) /
            Math.max(1, quadrantAverage)
        );
      ...
      const multiFit = ratioFit(
        multiRatio,
        currentDifficulty.targetMultiRatio,
        Math.max(0.035, currentDifficulty.targetMultiRatio * 1.1)
      );
      const strongFit = ratioFit(
        strongRatio,
        currentDifficulty.targetStrongRatio,
        Math.max(0.045, currentDifficulty.targetStrongRatio * 0.9)
      );

      // Zu viele gültige Leerfelder machen das Puzzle trivial. Dieser
      // Überschuss wird auf Hard/Expert besonders stark bestraft.
      const excessMoves = Math.max(
        0,
        moveRatio - currentDifficulty.targetMoveRatio
      );
      const excessPenalty =
        excessMoves *
        (selectedDifficultyKey === "easy"
    ...
    const generatePlayableBoard = (preset) => {
      ...
      for (
        let attempt = 0;
        attempt < currentDifficulty.candidateCount;
        attempt++
      ) {
        ...
        const requiredMoves = Math.max(
          2,
          Math.round(preset.minStartMoves * currentDifficulty.minMoveFactor)
        );
        const requiredRegions = Math.max(
          2,
          Math.round(preset.minMoveRegions * currentDifficulty.minRegionFactor)
        );
    ...
    const getOrphanIndices = (source) => {
      ...
      const orphanColors = new Set(
        counts
          .map((count, color) => (count === 1 ? color : null))
    ...
    const tryAddPreservedColorStep = (source, preset, remainingCounts) => {
      ...
      for (const option of shuffle(options)) {
        ...
        for (const pivot of shuffle(emptyPivots).slice(0, 100)) {
          ...
          const visibleColors = nearestAxisTiles(pivot, source, rows, cols).map(
            (tile) => tile.color
          );
    ...
    const generateRescueBoard = (preset, sourceBoard) => {
      ...
      if (counts.every((count) => count === 0))
        return Array(sourceBoard.length).fill(null);
      ...
      for (let attempt = 0; attempt < RESCUE_GENERATOR_CANDIDATES; attempt++) {
        ...
        const analysis = analyzeGeneratedBoard(generated.board, preset);
        const quality =
          analysis.quality + Math.min(12, analysis.moves.length * 0.45);
    ...
    const fitBoard = () => {
      ...
      const cellSize = Math.min(
        width / currentPreset.cols,
        height / currentPreset.rows
      );
      boardEl.style.setProperty(
        "--bb-symbol-size",
        `${Math.max(8, Math.min(18, cellSize * 0.52))}px`
      );
    };
    ...
    const visuallyRemoveCell = (cellIndex, power = 2) => {
      ...
      cell.classList.add(
        "removing",
        `power-${Math.max(2, Math.min(4, power))}`
      );
    ...
    const getComboMultiplier = (combo) =>
      COMBO_MULTIPLIERS[
        Math.min(Math.max(combo, 1), COMBO_MULTIPLIERS.length) - 1
      ];
    ...
    const getClearPercent = () => {
      ...
      return Math.max(
        ...
        Math.min(
          100,
          ((initialBlockCount - remainingBlocks) / initialBlockCount) * 100
        )
      );
    };
    ...
    const DEMO_SCENARIOS = [
      {
        ...
        explain:
          "Klicke das markierte leere Feld. Zwei gleiche Farben auf den Achsen verschwinden.",
        score: 100,
        clickIndex: 4,
      ...
      },
      {
        ...
        explain:
          "Sind drei der nächsten Achsen-Blöcke gleich, verschwinden alle drei auf einmal.",
        score: 250,
        clickIndex: 4,
      ...
      },
      {
        title: "Vierer-Match",
        explain:
          "Treffen sich dieselben Farben aus allen vier Richtungen, räumst du vier Blöcke mit einem Klick ab.",
        score: 500,
        clickIndex: 4,
      ...
      },
      {
        ...
        explain:
          "Auch zwei verschiedene Farbpaare zählen gleichzeitig: hier Blau horizontal und Orange vertikal.",
        score: 200,
        clickIndex: 4,
      ...
      },
      {
        ...
        explain:
          "Klickst du unten rechts, schaut das Spiel nach oben und links weiter, bis der nächste Block kommt. Die beiden blauen Eckblöcke bilden deshalb ein Paar.",
        score: 100,
        clickIndex: 8,
      ...
      },
      {
        ...
        explain:
          "Auch vom oberen linken Feld aus zählen die ersten sichtbaren Blöcke rechts und unten – selbst wenn mehrere leere Felder dazwischen liegen.",
        score: 100,
        clickIndex: 0,
    ...
    const renderDemoScenario = (scenario) => {
      ...
      demoStepEl.textContent = `Beispiel ${demoScenarioIndex + 1} / ${
      ...
      demoResultEl.innerHTML =
        "<span>●</span><strong>Klicke das markierte Feld</strong>";
    };
    ...
    const playDemoScenario = () => {
      ...
      demoTimeouts.push(
        setTimeout(() => {
          demoGridEl
            .querySelector(`[data-demo-index="${scenario.clickIndex}"]`)
            ?.classList.add("clicked");
          demoResultEl.innerHTML =
            "<span>◎</span><strong>Achsen werden geprüft …</strong>";
        }, 850)
      );
      ...
      demoTimeouts.push(
        setTimeout(() => {
          scenario.matched.forEach((index) =>
            demoGridEl
              .querySelector(`[data-demo-index="${index}"]`)
    ...
    const showRescueOverlay = (lostPoints, orphanCount = 0) => {
      const orphanText =
        orphanCount > 0
          ? ` · ${orphanCount} unpaarbare${
              orphanCount === 1 ? "r" : ""
            } Einzelblock${orphanCount === 1 ? "" : "e"} recycelt`
          : "";

      rescuePenaltyEl.textContent = `−${lostPoints.toLocaleString(
        "de-DE"
      )} Punkte${orphanText} · Farben bleiben erhalten`;
      ...
      setTimeout(() => {
        ...
        if (!rewired) {
          ...
          for (let attempt = 0; attempt < 70 && !fallback; attempt++) {
            const candidate = buildColorPreservingCandidate(
              currentPreset,
              getColorCounts(board)
            );
    ...
    const updateHUD = () => {
      ...
      if (comboCount > 0) {
        ...
        comboEl.textContent = `${comboCount} · ${multiplier.toLocaleString(
          "de-DE",
      ...
      const activeComboTier =
        comboTierClasses.find((className) =>
    ...
    const showPopup = (text, type, cellIndex, extraClass = "") => {
      ...
      if (cell) {
        ...
        popup.style.left = `${
          cellRect.left - areaRect.left + cellRect.width / 2
        }px`;
        popup.style.top = `${
          cellRect.top - areaRect.top + cellRect.height / 2
        }px`;
      }
    ...
    const endGame = (reason) => {
      ...
      if (reason === "cleared") {
        const rescueFactor = Math.pow(
          RESCUE_CLEAR_BONUS_MULTIPLIER,
          rescueCount
        );
        clearBonus = Math.max(
          0,
          Math.round(remainingTime * CLEAR_BONUS_PER_SECOND * rescueFactor)
        );
        score += clearBonus;
        remainingBlocks = 0;
        updateHUD();
      ...
      if (reason === "cleared") {
        ...
        } · Alle Blöcke entfernt · Zeitbonus +${clearBonus.toLocaleString(
          "de-DE"
        )} Punkte${rescueInfo}`;
      ...
      endAccuracyEl.textContent = `${accuracy.toLocaleString("de-DE", {
        maximumFractionDigits: 1,
      })}%`;
      endComboEl.textContent = biggestCombo.toLocaleString("de-DE");
      endClearEl.textContent = `${clearPercent.toLocaleString("de-DE", {
        maximumFractionDigits: 1,
      })}%`;
    ...
    const startRound = (presetKey = selectedPresetKey) => {
      ...
      score = 0;
      roundDuration = Math.round(
        currentPreset.seconds * currentDifficulty.timeMultiplier
      );
    ...
    const handleCellClick = (cellIndex) => {
      ...
      if (!result.valid) {
        ...
        remainingTime = Math.max(
          0,
          remainingTime - currentDifficulty.mistakePenalty
        );
      ...
      const multiMatchBonus =
        Math.max(0, result.groups.length - 1) *
        MULTI_MATCH_BONUS_PER_EXTRA_GROUP;
      const scoreBeforeCombo = result.score + multiMatchBonus;
      const gainedScore = Math.round(scoreBeforeCombo * multiplier);
      const matchPower = Math.max(
        ...result.groups.map((group) => group.indices.length)
      );
      ...
      if (multiplier > 1)
        popupParts.push(
          `${multiplier.toLocaleString("de-DE", {
            maximumFractionDigits: 1,
          })}× Combo`
        );
      if (streakReward)
        popupParts.push(`Hot Streak +${HOT_STREAK_TIME_BONUS}s`);
