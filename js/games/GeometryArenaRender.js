...
function drawWorld(ctx,world,lay,time){
  ...
  // Spawn telegraphs: compact, pulsing markers that show exactly where danger is about to enter.
  for(const sw of world.spawnWarnings||[]){
    const p=toScreen(lay,sw.x,sw.y),progress=clamp(1-sw.time/sw.maxTime,0,1),pulse=.55+.45*Math.sin((world.time||0)*18),base=(sw.boss?28:15)*scale,r=base*(1.45-.55*progress);
    ctx.save();ctx.globalCompositeOperation='lighter';ctx.strokeStyle=hexA(sw.boss?sw.color:'#ffffff',.42+.48*progress);ctx.lineWidth=Math.max(1.4,(sw.boss?3:2)*scale);ctx.shadowColor=sw.boss?sw.color:'#fff';ctx.shadowBlur=light>=3?(sw.boss?18:9):3;
    ctx.beginPath();ctx.arc(p.x,p.y,r,0,6.28);ctx.stroke();ctx.beginPath();ctx.arc(p.x,p.y,Math.max(3,r*.34),0,6.28);ctx.stroke();
    const arm=r*1.35,cut=r*.65;ctx.beginPath();ctx.moveTo(p.x-arm,p.y);ctx.lineTo(p.x-cut,p.y);ctx.moveTo(p.x+cut,p.y);ctx.lineTo(p.x+arm,p.y);ctx.moveTo(p.x,p.y-arm);ctx.lineTo(p.x,p.y-cut);ctx.moveTo(p.x,p.y+cut);ctx.lineTo(p.x,p.y+arm);ctx.stroke();
    ctx.fillStyle=hexA(sw.boss?sw.color:'#fff',.18+.18*pulse);ctx.beginPath();ctx.arc(p.x,p.y,Math.max(2,r*.22),0,6.28);ctx.fill();ctx.restore();
  }
  for(const e of world.enemies){const p=toScreen(lay,e.x,e.y),r=e.radius*scale;ctx.save();if(e.spawnGlow>0)ctx.globalAlpha=.62+.38*(1-e.spawnGlow/.35);ctx.shadowColor=e.color;ctx.shadowBlur=(light>=4?(e.boss?20:9):3)*(e.elite?1.25:1);ctx.lineWidth=Math.max(1,(e.boss?4:e.elite?3:2)*scale);ctx.strokeStyle=e.hit>0?'#fff':e.color;ctx.fillStyle=hexA(e.color,e.elite?.18:.08);enemyPath(ctx,e.shape,p.x,p.y,r,e.rot);ctx.fill();ctx.stroke();if(e.shape==='ring'){ctx.beginPath();ctx.arc(p.x,p.y,r*.55,0,6.28);ctx.stroke();}if(e.elite&&!e.boss){ctx.beginPath();ctx.arc(p.x,p.y,r*1.35,0,6.28);ctx.strokeStyle=hexA(e.color,.35);ctx.lineWidth=1;ctx.stroke();}ctx.restore();}
...
function drawRunRight(ctx,hotspots,run,world,lay,mode){
  ...
  const remaining=world&&mode==='battle'?Math.max(0,(world.waveTarget-world.spawnedEnemies)+world.enemies.length+(world.spawnWarnings?.filter(w=>!w.boss).length||0)+(world.bossStage&&!world.spawnBoss?1:0)):0;
