...
function drawProjectileBody(ctx,p,d,v){
  ...
  const bodyColor=p.customColor||d.color,shape=p.kind==="waterDrop"?"drop":p.kind==="sandGrain"||p.kind==="sandBurrow"?"sand":p.kind==="electronShard"?"electric":p.kind==="satelliteDive"?"satellite":p.kind==="phantomShell"?"ghost":p.kind==="castleStone"||p.kind==="slingRock"||p.kind==="slingHeavy"?"rock":p.kind==="crystalShard"?"crystal":v.shape;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(ang);ctx.shadowColor=bodyColor;ctx.shadowBlur=v.glow;ctx.fillStyle=bodyColor;ctx.strokeStyle=v.accent;ctx.lineWidth=Math.max(1,1.2*v.scale);
  switch(shape){
    ...
    case "balloon":ctx.rotate(-ang);ctx.beginPath();ctx.ellipse(0,0,r*1.05,r*1.25,0,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="rgba(255,255,255,.72)";ctx.beginPath();ctx.ellipse(-r*.34,-r*.40,r*.22,r*.38,-.35,0,Math.PI*2);ctx.fill();ctx.fillStyle=d.color;ctx.beginPath();ctx.moveTo(-r*.24,r*1.12);ctx.lineTo(r*.24,r*1.12);ctx.lineTo(0,r*1.52);ctx.closePath();ctx.fill();break;
    case "sand":ctx.fillStyle="#e1bc77";polygon(ctx,6,r*.92,.25);ctx.fill();ctx.strokeStyle="#fff0bd";ctx.globalAlpha=.55;ctx.stroke();break;
    case "dualroller":ctx.beginPath();ctx.arc(0,0,r*1.15,0,Math.PI*2);ctx.fill();ctx.stroke();for(let i=0;i<6;i++){ctx.strokeStyle=["#ff647c","#ffe56b","#68e8ff","#a986ff"][i%4];ctx.lineWidth=Math.max(1.3,r*.22);ctx.beginPath();ctx.arc(0,0,r*.72,i*Math.PI/3+.1,i*Math.PI/3+.75);ctx.stroke();}ctx.fillStyle="#f7ffff";ctx.beginPath();ctx.arc(0,0,r*.25,0,Math.PI*2);ctx.fill();break;
    case "imploder":ctx.rotate(-ang);for(let k=3;k>=1;k--){ctx.globalAlpha=.24+.18*k;ctx.strokeStyle=k===1?"#fff":d.color;ctx.lineWidth=1.2+k*.6;ctx.beginPath();ctx.arc(0,0,r*(.65+k*.42),0,Math.PI*2);ctx.stroke();}ctx.globalAlpha=1;ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(0,0,r*.55,0,Math.PI*2);ctx.fill();break;
    case "weightorb":ctx.rotate(-ang);ctx.strokeStyle="#dffaff";ctx.lineWidth=1.3;ctx.beginPath();ctx.moveTo(0,-r*1.65);ctx.lineTo(0,-r*.6);ctx.stroke();ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(0,r*.25,r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(-r*.32,-r*.08,r*.18,0,Math.PI*2);ctx.fill();break;
    case "satellite":ctx.fillStyle="#d8ebff";ctx.fillRect(-r*.55,-r*.48,r*1.1,r*.96);ctx.strokeRect(-r*.55,-r*.48,r*1.1,r*.96);ctx.fillStyle=d.color;ctx.fillRect(-r*1.55,-r*.55,r*.75,r*1.1);ctx.fillRect(r*.8,-r*.55,r*.75,r*1.1);ctx.strokeStyle="#fff";ctx.beginPath();ctx.moveTo(0,-r*.48);ctx.lineTo(r*.55,-r*1.2);ctx.stroke();break;
    case "portalcore":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=Math.max(2,r*.30);for(let k=0;k<3;k++){ctx.globalAlpha=.9-k*.22;ctx.beginPath();ctx.ellipse(0,0,r*(1.45-k*.30),r*(.72-k*.10),k*.55+p.age*2.5,0,Math.PI*2);ctx.stroke();}ctx.globalAlpha=1;ctx.fillStyle="#100722";ctx.beginPath();ctx.arc(0,0,r*.5,0,Math.PI*2);ctx.fill();break;
    case "domino":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.beginPath();ctx.roundRect(-r*.62,-r*1.18,r*1.24,r*2.36,r*.22);ctx.fill();ctx.stroke();ctx.fillStyle="#17212b";ctx.beginPath();ctx.arc(0,-r*.42,r*.16,0,Math.PI*2);ctx.arc(0,r*.42,r*.16,0,Math.PI*2);ctx.fill();break;
    case "hook":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=Math.max(3,r*.46);ctx.lineCap="round";ctx.beginPath();ctx.moveTo(0,-r*1.5);ctx.lineTo(0,r*.35);ctx.arc(-r*.48,r*.35,r*.48,0,Math.PI);ctx.stroke();ctx.lineCap="butt";break;
    case "castle":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.fillRect(-r*1.1,-r*.25,r*2.2,r*1.1);for(const x of [-.85,0,.85])ctx.fillRect(x*r-r*.23,-r*1.05,r*.46,r*.82);ctx.strokeStyle="#fff2c0";ctx.strokeRect(-r*1.1,-r*.25,r*2.2,r*1.1);break;
    case "compressor":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=Math.max(2,r*.34);ctx.beginPath();ctx.moveTo(-r*1.5,-r);ctx.lineTo(-r*.45,0);ctx.lineTo(-r*1.5,r);ctx.moveTo(r*1.5,-r);ctx.lineTo(r*.45,0);ctx.lineTo(r*1.5,r);ctx.stroke();ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(0,0,r*.24,0,Math.PI*2);ctx.fill();break;
    case "molecule":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(0,0,r*.62,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#dffcff";ctx.lineWidth=1;for(let k=0;k<3;k++){ctx.save();ctx.rotate(k*Math.PI/3);ctx.beginPath();ctx.ellipse(0,0,r*1.5,r*.52,0,0,Math.PI*2);ctx.stroke();ctx.restore();}break;
    case "beacon":ctx.rotate(-ang);ctx.fillStyle="#dbe5ed";ctx.beginPath();ctx.moveTo(-r*.6,r);ctx.lineTo(-r*.35,-r*.65);ctx.lineTo(r*.35,-r*.65);ctx.lineTo(r*.6,r);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(0,-r*.82,r*.42,0,Math.PI*2);ctx.fill();break;
    case "swap":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=Math.max(2,r*.35);ctx.beginPath();ctx.moveTo(-r*1.5,-r*.4);ctx.lineTo(r*.9,-r*.4);ctx.lineTo(r*.45,-r*.85);ctx.moveTo(r*1.5,r*.4);ctx.lineTo(-r*.9,r*.4);ctx.lineTo(-r*.45,r*.85);ctx.stroke();break;
    case "crystal":ctx.fillStyle=d.color;ctx.beginPath();ctx.moveTo(r*1.35,0);ctx.lineTo(r*.25,-r*1.15);ctx.lineTo(-r*.95,-r*.55);ctx.lineTo(-r*1.15,r*.42);ctx.lineTo(r*.1,r*1.10);ctx.closePath();ctx.fill();ctx.strokeStyle="#eaffff";ctx.beginPath();ctx.moveTo(r*.25,-r*1.15);ctx.lineTo(r*.1,r*1.1);ctx.stroke();break;
    case "blade":ctx.fillStyle="#dce8ef";ctx.beginPath();ctx.moveTo(r*1.9,-r*.25);ctx.lineTo(-r*1.4,-r*.45);ctx.lineTo(-r*1.7,r*.10);ctx.lineTo(r*1.55,r*.55);ctx.closePath();ctx.fill();ctx.strokeStyle=d.color;ctx.stroke();break;
    case "yoyo":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(0,0,r*1.08,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#10202b";ctx.beginPath();ctx.arc(0,0,r*.45,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#fff";ctx.beginPath();ctx.moveTo(0,-r*1.12);ctx.lineTo(0,-r*2);ctx.stroke();break;
    case "die":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.fillRect(-r,-r,r*2,r*2);ctx.strokeRect(-r,-r,r*2,r*2);ctx.fillStyle="#07131d";for(const [xx,yy] of [[-.4,-.4],[.4,.4],[0,0]]){ctx.beginPath();ctx.arc(xx*r,yy*r,1.2,0,Math.PI*2);ctx.fill();}break;
    case "turretseed":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.beginPath();ctx.moveTo(-r,r*.8);ctx.lineTo(0,-r);ctx.lineTo(r,r*.8);ctx.closePath();ctx.fill();ctx.stroke();break;
    case "poolball":ctx.rotate(-ang);ctx.fillStyle="#f4f4ec";ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(0,0,r*.48,0,Math.PI*2);ctx.fill();break;
    case "booster":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.fillRect(-r*.8,-r,r*1.6,r*2);ctx.strokeRect(-r*.8,-r,r*1.6,r*2);break;
    case "knight":ctx.rotate(-ang);ctx.font=`${Math.max(10,r*2.4)}px serif`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillStyle=d.color;ctx.fillText("♞",0,1);break;
    case "vortex":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=2;for(let i=0;i<3;i++){ctx.beginPath();ctx.arc(0,0,r*(.45+i*.35),i*.8,Math.PI*1.5+i*.8);ctx.stroke();}break;
    case "rocketwheel":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.stroke();for(let i=0;i<4;i++){ctx.rotate(Math.PI/2);ctx.fillRect(r*.4,-1,r*.8,2);}break;
    case "mine":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(0,0,r*.8,0,Math.PI*2);ctx.fill();for(let i=0;i<6;i++){ctx.rotate(Math.PI/3);ctx.fillRect(r*.65,-1,r*.65,2);}break;
    case "magnet":ctx.rotate(-ang);ctx.strokeStyle=d.color;ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,r,.25*Math.PI,.75*Math.PI,true);ctx.stroke();break;
    case "block":ctx.rotate(-ang);ctx.fillStyle=d.color;ctx.fillRect(-r,-r,r*2,r*2);ctx.strokeRect(-r,-r,r*2,r*2);break;
...
function drawFields(ctx,s){
  for(const f of s.fields){
    if(f.kind==="imploderCharge"){
      const t=clamp(f.life/f.max,0,1),charge=1-t;ctx.save();ctx.translate(f.x,f.y-8);ctx.shadowColor=f.color;ctx.shadowBlur=24+charge*24;for(let k=0;k<4;k++){ctx.strokeStyle=k===0?"#fff":f.color;ctx.globalAlpha=.28+.16*k;ctx.lineWidth=1.4+k*.7;ctx.beginPath();ctx.arc(0,0,18+charge*(f.r*.52)+k*9+Math.sin(f.life*16+k)*3,0,Math.PI*2);ctx.stroke();}ctx.globalAlpha=.95;ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(0,0,5+charge*7,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="dozerField"){
      const gy=terrainY(s,f.x),dir=f.dir||1;ctx.save();ctx.translate(f.x,gy-8);ctx.scale(dir,1);ctx.shadowColor=f.color;ctx.shadowBlur=15;
      ctx.fillStyle="#17232d";ctx.strokeStyle=f.color;ctx.lineWidth=2;ctx.beginPath();ctx.roundRect(-20,-10,34,14,5);ctx.fill();ctx.stroke();
      ctx.fillStyle="#0b1118";for(const ox of [-12,0,12]){ctx.beginPath();ctx.arc(ox,3,5,0,Math.PI*2);ctx.fill();ctx.stroke();}
      ctx.fillStyle=f.color;ctx.globalAlpha=.85;ctx.beginPath();ctx.moveTo(13,-5);ctx.lineTo(29,-13);ctx.lineTo(31,8);ctx.lineTo(15,7);ctx.closePath();ctx.fill();ctx.strokeStyle="#f4ffff";ctx.globalAlpha=.7;ctx.stroke();
      ctx.globalAlpha=.42;for(let i=0;i<4;i++){ctx.fillStyle=i%2?"#b88952":"#70513c";ctx.beginPath();ctx.arc(29+i*5,5-i*2,3+i*.5,0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="plinkoBoard"){
      const top=f.top||80,rowH=(f.y-top-32)/Math.max(1,f.rows),step=f.span/Math.max(3,f.rows+2);ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=10;
      ctx.strokeStyle="rgba(173,231,255,.35)";ctx.lineWidth=1.4;ctx.beginPath();ctx.roundRect(f.x-f.span*.53,top-14,f.span*1.06,Math.max(50,f.y-top+12),12);ctx.stroke();
      for(let r=0;r<f.rows;r++){const yy=top+34+r*rowH,n=Math.max(3,Math.floor(f.span/step));for(let i=0;i<n;i++){const xx=f.x+(i-(n-1)/2)*step+(r%2?step*.5:0);if(Math.abs(xx-f.x)>f.span*.48)continue;ctx.fillStyle=r%2?"#a6f2ff":"#d6b1ff";ctx.globalAlpha=.6;ctx.beginPath();ctx.arc(xx,yy,2.7,0,Math.PI*2);ctx.fill();}}
      ctx.globalAlpha=1;for(const b of f.balls||[]){if(b.done)continue;ctx.fillStyle=b.gold?"#ffe66b":f.color;ctx.strokeStyle="#ffffff";ctx.lineWidth=1.3;ctx.shadowColor=b.gold?"#ffe66b":f.color;ctx.shadowBlur=14;ctx.beginPath();ctx.arc(b.x??f.x,b.y??top,6.2,0,Math.PI*2);ctx.fill();ctx.stroke();}
      ctx.restore();
    }else if(f.kind==="aegisDome"){
      const cy=f.y-5,r=f.r||70,pct=(f.maxCharges?f.charges/f.maxCharges:0);ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=19;
      const g=ctx.createRadialGradient(f.x,cy,8,f.x,cy,r);g.addColorStop(0,(f.color||"#7cecff")+"12");g.addColorStop(.7,(f.color||"#7cecff")+"20");g.addColorStop(1,(f.color||"#7cecff")+"05");ctx.fillStyle=g;ctx.beginPath();ctx.arc(f.x,cy,r,Math.PI,0);ctx.lineTo(f.x+r,cy+7);ctx.lineTo(f.x-r,cy+7);ctx.closePath();ctx.fill();
      ctx.strokeStyle=f.color;ctx.lineWidth=2.8;ctx.globalAlpha=.55+.25*Math.sin(performance.now()/170);ctx.beginPath();ctx.arc(f.x,cy,r,Math.PI,0);ctx.stroke();
      ctx.globalAlpha=.8;ctx.lineWidth=1;for(let i=1;i<=3;i++){ctx.beginPath();ctx.arc(f.x,cy,r*i/4,Math.PI,0);ctx.stroke();}
      const n=Math.max(1,f.maxCharges||1);for(let i=0;i<n;i++){const a=Math.PI+(i+.5)/n*Math.PI,xx=f.x+Math.cos(a)*(r+10),yy=cy+Math.sin(a)*(r+10);ctx.fillStyle=i<(f.charges||0)?"#edffff":"#24333d";ctx.beginPath();ctx.arc(xx,yy,3.2,0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="gravityLasso"){
      ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=15;ctx.lineWidth=2;ctx.setLineDash([7,4]);for(let i=0;i<(f.targetIds||[]).length;i++){const tx=f[`tx${i}`]??f.x,ty=f[`ty${i}`]??f.y;ctx.beginPath();ctx.moveTo(f.x,f.y);ctx.quadraticCurveTo((f.x+tx)/2,Math.min(f.y,ty)-38,tx,ty);ctx.stroke();ctx.globalAlpha=.38;ctx.beginPath();ctx.arc(f.x,f.y,f.orbit||50,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;ctx.beginPath();ctx.arc(tx,ty,10,0,Math.PI*2);ctx.stroke();}ctx.setLineDash([]);ctx.fillStyle="#f4ffff";ctx.beginPath();ctx.arc(f.x,f.y,5,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="laserPlow"){
      ctx.save();ctx.globalAlpha=clamp(f.life/f.max,0,1);for(const r of f.rays||[]){const g=ctx.createLinearGradient(r.ax,r.ay,r.bx,r.by);g.addColorStop(0,"rgba(255,255,255,.9)");g.addColorStop(.18,f.color);g.addColorStop(1,(f.color||"#fff")+"22");ctx.strokeStyle=g;ctx.shadowColor=f.color;ctx.shadowBlur=22;ctx.lineWidth=(f.width||10)*1.45;ctx.beginPath();ctx.moveTo(r.ax,r.ay);ctx.lineTo(r.bx,r.by);ctx.stroke();ctx.strokeStyle="#ffffff";ctx.shadowBlur=6;ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(r.ax,r.ay);ctx.lineTo(r.bx,r.by);ctx.stroke();}ctx.restore();
    }else if(f.kind==="conveyorBelt"){
      const a=Math.max(5,f.x-f.span*.5),b=Math.min(s.width-5,f.x+f.span*.5),tm=performance.now()/170;ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=10;ctx.lineWidth=5;ctx.globalAlpha=.56;ctx.beginPath();for(let xx=a;xx<=b;xx+=4){const yy=terrainY(s,xx)-2;xx===a?ctx.moveTo(xx,yy):ctx.lineTo(xx,yy);}ctx.stroke();ctx.globalAlpha=.9;for(let xx=a+12;xx<b-8;xx+=25){const dir=f.mode==="split"?Math.sign(f.x-xx||f.dir):f.dir,yy=terrainY(s,xx)-7,phase=((tm+xx*.03)%1);ctx.save();ctx.translate(xx+dir*phase*8,yy);ctx.scale(dir,1);ctx.fillStyle="#eaffff";ctx.beginPath();ctx.moveTo(7,0);ctx.lineTo(-3,-4);ctx.lineTo(-3,4);ctx.closePath();ctx.fill();ctx.restore();}ctx.restore();
    }else if(f.kind==="geoStamp"){
      const progress=1-clamp(f.life/f.max,0,1),drop=Math.sin(Math.min(1,(progress*f.presses)%1)*Math.PI),yy=terrainY(s,f.x)-42-drop*24;ctx.save();ctx.translate(f.x,yy);ctx.shadowColor=f.color;ctx.shadowBlur=18;ctx.fillStyle="rgba(18,30,40,.72)";ctx.strokeStyle=f.color;ctx.lineWidth=2.4;ctx.beginPath();ctx.roundRect(-f.span*.36,-18,f.span*.72,22,5);ctx.fill();ctx.stroke();const teeth=Math.max(2,f.teeth||3);for(let i=0;i<teeth;i++){const xx=(i-(teeth-1)/2)*(f.span*.64/Math.max(1,teeth-1));ctx.fillStyle=i%2?"#efffff":f.color;ctx.beginPath();ctx.moveTo(xx-6,4);ctx.lineTo(xx+6,4);ctx.lineTo(xx,18+(i%2)*7);ctx.closePath();ctx.fill();}ctx.globalAlpha=.35;ctx.strokeStyle="#fff";ctx.beginPath();ctx.moveTo(-f.span*.42,26);ctx.lineTo(f.span*.42,26);ctx.stroke();ctx.restore();
    }else if(f.kind==="newtonCradle"){
      const balls=f.balls||[];if(balls.length){const top=Math.min(...balls.map(q=>q.y))-48,sw=f.swing||0;ctx.save();ctx.strokeStyle="#dce8ef";ctx.shadowColor=f.color;ctx.shadowBlur=10;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(balls[0].x-12,top);ctx.lineTo(balls[balls.length-1].x+12,top);ctx.stroke();for(let i=0;i<balls.length;i++){const q=balls[i],edge=i===0?-1:i===balls.length-1?1:0,shift=edge*sw*18;ctx.strokeStyle="rgba(220,238,247,.65)";ctx.beginPath();ctx.moveTo(q.x,top);ctx.lineTo(q.x+shift,q.y);ctx.stroke();ctx.fillStyle=edge?f.color:"#cad5dc";ctx.strokeStyle="#fff";ctx.beginPath();ctx.arc(q.x+shift,q.y,8.5,0,Math.PI*2);ctx.fill();ctx.stroke();}ctx.restore();}
    }else if(f.kind==="helixRise"){
      const h=(f.height||120)*(f.progress||0),strands=Math.max(2,f.strands||2);ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=16;ctx.lineWidth=2.6;for(let k=0;k<strands;k++){ctx.strokeStyle=k%2?f.color:"#c7ffff";ctx.beginPath();for(let i=0;i<=34;i++){const q=i/34,y=f.y-q*h,a=q*(f.turns||1.5)*Math.PI*2+k/strands*Math.PI*2,x=f.x+Math.sin(a)*22*(.35+.65*q);i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.stroke();}for(let i=0;i<7;i++){const q=i/6,y=f.y-q*h,a=q*(f.turns||1.5)*Math.PI*2,x1=f.x+Math.sin(a)*22*(.35+.65*q),x2=f.x+Math.sin(a+Math.PI)*22*(.35+.65*q);ctx.globalAlpha=.3;ctx.strokeStyle="#fff";ctx.beginPath();ctx.moveTo(x1,y);ctx.lineTo(x2,y);ctx.stroke();}ctx.restore();
    }else if(f.kind==="anchorChain"){
      ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=10;ctx.lineWidth=2.2;for(const id of f.targetIds||[]){const t=s.tanks.find(q=>q.id===id&&q.alive);if(!t)continue;const mx=(f.x+t.x)/2,my=Math.min(f.y,t.y)-16;ctx.beginPath();ctx.moveTo(f.x,f.y);ctx.quadraticCurveTo(mx,my,t.x,t.y);ctx.stroke();for(let i=1;i<6;i++){const q=i/6,x=(1-q)*(1-q)*f.x+2*(1-q)*q*mx+q*q*t.x,y=(1-q)*(1-q)*f.y+2*(1-q)*q*my+q*q*t.y;ctx.fillStyle="#d8e2e8";ctx.beginPath();ctx.arc(x,y,2.3,0,Math.PI*2);ctx.fill();}}ctx.fillStyle="#1d2830";ctx.strokeStyle="#f0f6fa";ctx.beginPath();ctx.moveTo(f.x,f.y-15);ctx.lineTo(f.x,f.y+5);ctx.moveTo(f.x-10,f.y-2);ctx.quadraticCurveTo(f.x,f.y+12,f.x+10,f.y-2);ctx.stroke();ctx.restore();
    }else if(f.kind==="landSlide"){
      const a=Math.max(4,f.x-f.span*.5),b=Math.min(s.width-4,f.x+f.span*.5),u=1-clamp(f.life/f.max,0,1);ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.globalAlpha=.22+.20*Math.sin(u*Math.PI*10);for(let k=0;k<4;k++){ctx.beginPath();for(let xx=a;xx<=b;xx+=8){const yy=terrainY(s,xx)-5-k*4-Math.sin(xx*.06+u*12+k)*3;xx===a?ctx.moveTo(xx,yy):ctx.lineTo(xx,yy);}ctx.stroke();}for(let i=0;i<10;i++){const xx=a+((i*.137+u*.7)%1)*(b-a),yy=terrainY(s,xx)-9-(i%3)*5;ctx.fillStyle=i%2?"#a98869":"#796153";ctx.beginPath();ctx.arc(xx,yy,2.5+i%2,0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="bubbleLift"){
      ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=14;for(let i=0;i<(f.targetIds||[]).length;i++){const x=f[`tx${i}`]??f.x,y=f[`ty${i}`]??f.y,r=24+i*2;const g=ctx.createRadialGradient(x-r*.3,y-r*.3,2,x,y,r);g.addColorStop(0,"rgba(255,255,255,.32)");g.addColorStop(.35,(f.color||"#7defff")+"24");g.addColorStop(1,"rgba(100,220,255,.03)");ctx.fillStyle=g;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.lineWidth=2;ctx.globalAlpha=.75;ctx.stroke();ctx.globalAlpha=.8;ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(x-r*.35,y-r*.4,3,0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="bridgeBuild"){
      const a=Math.min(f.a,f.b),b=Math.max(f.a,f.b),u=1-clamp(f.life/f.max,0,1);ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=11;ctx.lineWidth=2;ctx.globalAlpha=.45+.35*(1-u);ctx.beginPath();for(let xx=a;xx<=b;xx+=5){const yy=terrainY(s,xx)-5;xx===a?ctx.moveTo(xx,yy):ctx.lineTo(xx,yy);}ctx.stroke();const n=Math.max(2,f.supports||3);for(let i=0;i<n;i++){const xx=a+(i+.5)/n*(b-a),yy=terrainY(s,xx)-4;ctx.globalAlpha=.35;ctx.beginPath();ctx.moveTo(xx,yy);ctx.lineTo(xx,Math.min(s.height-6,yy+24+((i%2)*12)));ctx.stroke();ctx.beginPath();ctx.moveTo(xx-10,yy+18);ctx.lineTo(xx+10,yy+18);ctx.stroke();}ctx.restore();
    }else if(f.kind==="dualRollerField"){
      const yy=terrainY(s,f.x)-5;ctx.save();ctx.translate(f.x,yy);ctx.rotate((f.max-f.life)*f.dir*8);ctx.shadowColor=f.color;ctx.shadowBlur=16;for(let i=0;i<6;i++){ctx.strokeStyle=["#63e6ff","#ff6d8e","#ffe66b","#9d82ff"][i%4];ctx.lineWidth=2.2;ctx.beginPath();ctx.arc(0,0,9,i*Math.PI/3+.1,i*Math.PI/3+.8);ctx.stroke();}ctx.fillStyle="#f4ffff";ctx.beginPath();ctx.arc(0,0,2.3,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="pendulumField"){
      const anchorY=f.anchorY??(f.y-f.rope),elapsed=f.max-f.life,wave=elapsed/f.max*f.passes*Math.PI*2,maxAngle=Math.asin(Math.min(.92,f.span/f.rope));ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=14;for(let i=0;i<f.orbs;i++){const cached=f[`orb${i}`],theta=Math.sin(wave+i/f.orbs*Math.PI*2)*maxAngle,px=cached?.x??f.x+Math.sin(theta)*f.rope,py=cached?.y??anchorY+Math.cos(theta)*f.rope;ctx.strokeStyle="rgba(222,248,255,.72)";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(f.x,anchorY);ctx.lineTo(px,py);ctx.stroke();ctx.fillStyle=f.color;ctx.beginPath();ctx.arc(px,py,11,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#fff";ctx.stroke();}ctx.fillStyle="#e8fbff";ctx.beginPath();ctx.arc(f.x,anchorY,5,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="teslaGate"){
      const n=f.pylons,pts=[];for(let i=0;i<n;i++){const q=n<=1?.5:i/(n-1),xx=clamp(f.x+(q-.5)*f.span,6,s.width-6);pts.push({x:xx,y:terrainY(s,xx)-(f.beamHeight||12)});}ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=18;for(const q of pts){ctx.strokeStyle=f.color;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(q.x,terrainY(s,q.x));ctx.lineTo(q.x,q.y-18);ctx.stroke();ctx.fillStyle="#efffcf";ctx.beginPath();ctx.arc(q.x,q.y,4,0,Math.PI*2);ctx.fill();}ctx.strokeStyle="#eaff99";ctx.lineWidth=3;ctx.setLineDash([6,4]);for(let i=0;i<pts.length-1;i++){ctx.beginPath();ctx.moveTo(pts[i].x,pts[i].y);ctx.lineTo(pts[i+1].x,pts[i+1].y);ctx.stroke();}if(f.cross&&pts.length>=4){ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);ctx.lineTo(pts[2].x,pts[2].y);ctx.moveTo(pts[1].x,pts[1].y);ctx.lineTo(pts[3].x,pts[3].y);ctx.stroke();}ctx.setLineDash([]);ctx.restore();
    }else if(f.kind==="satelliteOrbit"){
      const elapsed=f.max-f.life,rot=elapsed*2.7;ctx.save();ctx.translate(f.x,f.y);ctx.strokeStyle=f.color;ctx.globalAlpha=.35;ctx.beginPath();ctx.ellipse(0,0,f.orbitRadius,f.orbitRadius*.45,0,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;for(let i=0;i<f.count;i++){const a=rot+i/f.count*Math.PI*2,x=Math.cos(a)*f.orbitRadius,y=Math.sin(a)*f.orbitRadius*.45;ctx.save();ctx.translate(x,y);ctx.fillStyle="#d9edff";ctx.fillRect(-4,-3,8,6);ctx.fillStyle=f.color;ctx.fillRect(-10,-3,5,6);ctx.fillRect(5,-3,5,6);ctx.restore();}ctx.restore();
    }else if(f.kind==="portalCollapse"){
      const t=clamp(f.life/f.max,0,1);ctx.save();ctx.translate(f.x,f.y);ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=20;for(let i=0;i<3;i++){ctx.globalAlpha=.75-i*.18;ctx.lineWidth=2+i;ctx.beginPath();ctx.ellipse(0,0,(22+i*11)*(1.2-t*.3),(9+i*5)*(1.2-t*.3),f.life*3+i*.5,0,Math.PI*2);ctx.stroke();}ctx.restore();
    }else if(f.kind==="prismCage"){
      const pts=[];for(let i=0;i<f.sides;i++){const a=-Math.PI/2+i/f.sides*Math.PI*2;pts.push({x:f.x+Math.cos(a)*f.span,y:f.y-f.height*.42+Math.sin(a)*f.height*.50});}ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=13;ctx.lineWidth=1.7;ctx.globalAlpha=.65;ctx.beginPath();pts.forEach((q,i)=>i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y));ctx.closePath();ctx.stroke();for(const q of pts){ctx.fillStyle="#eaffff";ctx.beginPath();ctx.arc(q.x,q.y,3.5,0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="dominoChain"){
      ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=10;for(let i=0;i<f.count;i++){const xx=clamp(f.x+(i-(f.count-1)/2)*f.spacing,5,s.width-5),yy=terrainY(s,xx)-5,done=f.returning?i>=f.count-f.index:i<f.index,next=f.returning?i===f.count-1-f.index:i===f.index;ctx.save();ctx.translate(xx,yy);ctx.rotate(next?(f.returning?-.55:.55):done?(f.returning?-.9:.9):0);ctx.globalAlpha=done?.28:.90;ctx.fillStyle=done?"#6b5635":f.color;ctx.fillRect(-5,-18,10,18);ctx.strokeStyle="#fff2c2";ctx.strokeRect(-5,-18,10,18);ctx.restore();}ctx.restore();
    }else if(f.kind==="skyHook"){
      const tx=f.targetX??f.startX,ty=f.targetY??terrainY(s,tx);ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(tx,0);ctx.lineTo(tx,ty-12);ctx.stroke();ctx.strokeStyle="#e8f8ff";ctx.lineWidth=4;ctx.beginPath();ctx.arc(tx,ty-7,8,0,Math.PI*.9);ctx.stroke();ctx.restore();
    }else if(f.kind==="castleDrop"){
      const gy=terrainY(s,f.x);ctx.save();ctx.fillStyle=f.color;ctx.strokeStyle="#fff1b9";ctx.globalAlpha=.76;for(const off of [-f.span*.48,f.span*.48]){const x=f.x+off,y=terrainY(s,x);ctx.fillRect(x-8,y-42,16,42);ctx.strokeRect(x-8,y-42,16,42);for(let k=-1;k<=1;k++)ctx.fillRect(x+k*7-3,y-49,6,9);}ctx.restore();
    }else if(f.kind==="compressor"){
      const left=f.left??f.x-f.span,right=f.right??f.x+f.span,gy=terrainY(s,f.x);ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=18;for(const wx of [left,right]){const g=ctx.createLinearGradient(wx-8,0,wx+8,0);g.addColorStop(0,"rgba(255,100,185,0)");g.addColorStop(.5,f.color);g.addColorStop(1,"rgba(255,100,185,0)");ctx.fillStyle=g;ctx.globalAlpha=.72;ctx.fillRect(wx-10,70,20,Math.max(30,gy-70));ctx.strokeStyle="#fff";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(wx,80);ctx.lineTo(wx,gy);ctx.stroke();}ctx.restore();
    }else if(f.kind==="pinballRig"){
      ctx.save();ctx.strokeStyle="rgba(255,112,214,.42)";ctx.lineWidth=1.4;ctx.beginPath();for(let i=0;i<f.nodes.length;i++){const a=f.nodes[i],b=f.nodes[(i+1)%f.nodes.length];ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);}ctx.stroke();for(const q of f.nodes){ctx.fillStyle="#172230";ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.lineWidth=3;ctx.beginPath();ctx.arc(q.x,q.y,10,0,Math.PI*2);ctx.fill();ctx.stroke();}ctx.fillStyle="#fff";ctx.shadowBlur=18;ctx.beginPath();ctx.arc(f.ballX??f.x,f.ballY??f.y,5.5,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="lighthouse"){
      const a=f.angle||0,gy=terrainY(s,f.x),top=gy-48;ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=14;ctx.fillStyle="#d7e0e8";ctx.beginPath();ctx.moveTo(f.x-10,gy);ctx.lineTo(f.x-6,top+6);ctx.lineTo(f.x+6,top+6);ctx.lineTo(f.x+10,gy);ctx.closePath();ctx.fill();ctx.strokeStyle=f.color;ctx.stroke();ctx.fillStyle="#122335";ctx.fillRect(f.x-8,top-7,16,10);ctx.strokeRect(f.x-8,top-7,16,10);ctx.fillStyle=f.solar?"#fff5b5":"#eaffff";ctx.beginPath();ctx.arc(f.x,top-2,5,0,Math.PI*2);ctx.fill();for(let b=0;b<f.beams;b++){const dir=f.counter&&b%2?-1:1,aa=dir*a+b/f.beams*Math.PI*2,bx=f.x+Math.cos(aa)*f.range,by=top+Math.sin(aa)*f.range;ctx.save();const grad=ctx.createLinearGradient(f.x,top,bx,by);grad.addColorStop(0,f.solar?"rgba(255,244,160,.55)":"rgba(235,255,220,.48)");grad.addColorStop(.55,f.solar?"rgba(255,220,120,.17)":"rgba(170,245,255,.13)");grad.addColorStop(1,"rgba(255,255,255,0)");ctx.strokeStyle=grad;ctx.lineWidth=(f.beamWidth||15)*1.6;ctx.globalAlpha=.75;ctx.beginPath();ctx.moveTo(f.x,top);ctx.lineTo(bx,by);ctx.stroke();ctx.globalAlpha=.95;ctx.strokeStyle=f.color;ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(f.x,top);ctx.lineTo(bx,by);ctx.stroke();ctx.restore();}ctx.globalAlpha=.35;ctx.strokeStyle=f.color;for(let k=0;k<3;k++){ctx.beginPath();ctx.arc(f.x,top,15+k*9+(Math.sin((f.max-f.life)*5+k)*3),0,Math.PI*2);ctx.stroke();}ctx.restore();
    }else if(f.kind==="repulsor"){
      const elapsed=f.max-f.life,phase=(elapsed/.27)%1,r=f.range*phase;ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=14;ctx.globalAlpha=.55;ctx.lineWidth=3;ctx.beginPath();ctx.arc(f.x,f.y,r,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=.9;ctx.fillStyle="#eaffff";ctx.beginPath();ctx.arc(f.x,f.y,5,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="phantomRift"){
      ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=18;ctx.lineWidth=3;ctx.globalAlpha=.75;for(let k=0;k<3;k++){ctx.beginPath();ctx.ellipse(f.x,f.y,14+k*9,6+k*4,f.life*2+k*.5,0,Math.PI*2);ctx.stroke();}ctx.restore();
    }else if(f.kind==="razorHalo"){
      const rad=f.radius??f.start,rot=f.angle||0;ctx.save();ctx.translate(f.x,f.y);ctx.shadowColor=f.color;ctx.shadowBlur=12;for(let i=0;i<f.blades;i++){const a=rot+i/f.blades*Math.PI*2,x=Math.cos(a)*rad,y=Math.sin(a)*rad*.58;ctx.save();ctx.translate(x,y);ctx.rotate(a+Math.PI/2+(f.max-f.life)*7);ctx.fillStyle="#dce5eb";ctx.beginPath();for(let k=0;k<8;k++){ctx.rotate(Math.PI/4);ctx.moveTo(5,-2);ctx.lineTo(11,0);ctx.lineTo(5,2);}ctx.fill();ctx.strokeStyle=f.color;ctx.beginPath();ctx.arc(0,0,6,0,Math.PI*2);ctx.stroke();ctx.restore();}ctx.restore();
    }else if(f.kind==="crystalBloom"){
      const u=clamp((f.max-f.life)/.48,0,1);ctx.save();ctx.shadowColor=f.color;ctx.shadowBlur=14;for(let i=0;i<f.spikes;i++){const q=f.spikes<=1?.5:i/(f.spikes-1),xx=f.x+(q-.5)*f.span,yy=terrainY(s,xx),h=(22+12*(i%3))*u;ctx.fillStyle=i%2?"#74dfff":f.color;ctx.beginPath();ctx.moveTo(xx-8,yy);ctx.lineTo(xx,yy-h);ctx.lineTo(xx+8,yy);ctx.closePath();ctx.fill();ctx.strokeStyle="#eaffff";ctx.stroke();}ctx.restore();
    }else if(f.kind==="guillotine"){
      ctx.save();ctx.translate(f.x,f.y);ctx.rotate(f.angle||0);ctx.shadowColor=f.color;ctx.shadowBlur=18;ctx.fillStyle=f.pattern==="verdict"?"#fff4f0":"#dce8ef";ctx.strokeStyle=f.pattern==="verdict"?"#ff647d":f.color;ctx.lineWidth=2.5;const l=f.length;ctx.beginPath();ctx.moveTo(-l/2,-6);ctx.lineTo(l/2,-6);ctx.lineTo(l/2-12,8);ctx.lineTo(-l/2+9,6);ctx.closePath();ctx.fill();ctx.stroke();if(f.pattern==="verdict"){ctx.globalAlpha=.35;ctx.strokeStyle="#ff5e7c";ctx.beginPath();ctx.arc(0,0,22,0,Math.PI*2);ctx.stroke();}ctx.restore();
    }else if(f.kind==="yoyoField"){
      const bx=f.ballX??f.anchorX,by=f.ballY??f.anchorY;ctx.save();ctx.strokeStyle="rgba(255,210,240,.58)";ctx.lineWidth=1.7;ctx.beginPath();ctx.moveTo(f.anchorX,f.anchorY);ctx.lineTo(bx,by);ctx.stroke();ctx.shadowColor=f.color;ctx.shadowBlur=18;ctx.fillStyle=f.color;ctx.beginPath();ctx.arc(bx,by,8,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#fff";ctx.lineWidth=2;ctx.beginPath();ctx.arc(bx,by,3,0,Math.PI*2);ctx.stroke();ctx.restore();

    }else if(f.kind==="diceCore"){
      ctx.save();ctx.translate(f.x,f.y);ctx.rotate((f.max-f.life)*2.2);ctx.shadowColor=f.color;ctx.shadowBlur=22;ctx.fillStyle="#12233a";ctx.strokeStyle=f.color;ctx.lineWidth=3;ctx.fillRect(-18,-18,36,36);ctx.strokeRect(-18,-18,36,36);ctx.rotate(-(f.max-f.life)*2.2);ctx.fillStyle="#eaffff";ctx.font="bold 18px system-ui";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(String(f.lastFace||1),0,1);if(f.triple){ctx.globalAlpha=.30;ctx.strokeStyle="#ffd56f";ctx.strokeRect(-26,-26,52,52);}ctx.restore();
    }else if(f.kind==="sentryBattery"){
      ctx.save();for(const q of f.turrets){ctx.translate(q.x,q.y);ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.fillStyle="#152838";ctx.strokeStyle=f.color;ctx.fillRect(-9,-8,18,8);ctx.strokeRect(-9,-8,18,8);ctx.fillStyle=f.color;ctx.fillRect(-3,-17,6,10);ctx.fillRect(2,-15,14,3);if(q.flash){ctx.fillStyle="#fff7b3";ctx.beginPath();ctx.arc(18,-13,4,0,Math.PI*2);ctx.fill();}ctx.translate(-q.x,-q.y);}ctx.restore();
    }else if(f.kind==="poolTable"){
      const lo=f.x-f.span*.62,hi=f.x+f.span*.62;ctx.save();if(f.rails){ctx.strokeStyle="#ff72d8";ctx.shadowColor="#ff72d8";ctx.shadowBlur=12;ctx.lineWidth=3;for(const xx of [lo,hi]){ctx.beginPath();ctx.moveTo(xx,terrainY(s,xx)-80);ctx.lineTo(xx,terrainY(s,xx));ctx.stroke();}}for(const b of f.balls){ctx.fillStyle=b.id%2?"#ffe36b":"#f5f5ef";ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=7;ctx.beginPath();ctx.arc(b.x,b.y,b.r||5,0,Math.PI*2);ctx.fill();ctx.stroke();}ctx.restore();
    }else if(f.kind==="launchPad"){
      ctx.save();ctx.strokeStyle=f.color;ctx.fillStyle="#173044";ctx.shadowColor=f.color;ctx.shadowBlur=16;ctx.fillRect(f.x-24,terrainY(s,f.x)-7,48,7);ctx.strokeRect(f.x-24,terrainY(s,f.x)-7,48,7);for(const id of f.targetIds){const t=s.tanks.find(q=>q.id===id);if(t){ctx.globalAlpha=.45;ctx.fillStyle=f.color;ctx.fillRect(t.x-8,t.y+10,16,terrainY(s,t.x)-t.y-8);}}ctx.restore();
    }else if(f.kind==="knightField"){
      ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.globalAlpha=.5;for(const path of f.paths){ctx.beginPath();path.forEach((q,i)=>i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y));ctx.stroke();}const idx=Math.min(f.index,Math.max(...f.paths.map(a=>a.length))-1);for(const path of f.paths){const q=path[Math.max(0,idx)];if(q){ctx.globalAlpha=1;ctx.font="26px serif";ctx.fillStyle=f.color;ctx.fillText("♞",q.x-12,q.y-4);}}ctx.restore();
    }else if(f.kind==="cycloneField"){
      const u=1-f.life/f.max;ctx.save();for(let c=0;c<f.count;c++){const dir=c%2?1:-1,cx=f.x+dir*Math.sin(u*Math.PI*2)*f.range*.55,cy=terrainY(s,cx)-26;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=14;for(let k=0;k<5;k++){ctx.globalAlpha=.75-k*.1;ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(cx,cy-k*8,12+k*6,5+k*2,u*8+k,0,Math.PI*2);ctx.stroke();}}ctx.restore();
    }else if(f.kind==="rocketCarousel"){
      const a=f.angle||0;ctx.save();ctx.translate(f.x,f.y);ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=15;ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,f.orbit,0,Math.PI*2);ctx.stroke();for(let i=0;i<f.pods;i++){const aa=a+i/f.pods*Math.PI*2,x=Math.cos(aa)*f.orbit,y=Math.sin(aa)*f.orbit*.45;ctx.save();ctx.translate(x,y);ctx.rotate(aa);ctx.fillStyle="#e9f3f7";ctx.fillRect(-6,-3,12,6);ctx.fillStyle=f.color;ctx.fillRect(4,-2,7,4);ctx.restore();}ctx.restore();
    }else if(f.kind==="prismBank"){
      const pts=[f.start,...f.nodes,f.end];ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=20;ctx.lineWidth=3;ctx.beginPath();pts.forEach((q,i)=>i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y));ctx.stroke();for(const q of f.nodes){ctx.save();ctx.translate(q.x,q.y);ctx.rotate(Math.PI/4);ctx.fillStyle="#102737";ctx.strokeStyle="#eaffff";ctx.fillRect(-7,-7,14,14);ctx.strokeRect(-7,-7,14,14);ctx.restore();}ctx.restore();
    }else if(f.kind==="proximityMine"){
      ctx.save();ctx.translate(f.x,f.y);ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.fillStyle="#18232c";ctx.strokeStyle=f.color;ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,7,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.globalAlpha=.28+.15*Math.sin(performance.now()/160);ctx.beginPath();ctx.arc(0,0,f.sensor,0,Math.PI*2);ctx.stroke();ctx.restore();
    }else if(f.kind==="icePatch"){
      ctx.save();ctx.strokeStyle=f.color;ctx.fillStyle="rgba(105,225,255,.16)";ctx.shadowColor=f.color;ctx.shadowBlur=8;ctx.beginPath();const a=Math.max(3,f.x-f.span*.5),b=Math.min(s.width-3,f.x+f.span*.5);ctx.moveTo(a,terrainY(s,a));for(let xx=a;xx<=b;xx+=5)ctx.lineTo(xx,terrainY(s,xx)-2);for(let xx=b;xx>=a;xx-=5)ctx.lineTo(xx,terrainY(s,xx)+6);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();
    }else if(f.kind==="magnetron"){
      ctx.save();ctx.translate(f.x,f.y);ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=16;ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,24,.25*Math.PI,.75*Math.PI,true);ctx.stroke();ctx.lineWidth=2;ctx.globalAlpha=.35;ctx.beginPath();ctx.arc(0,0,f.range*(.45+.12*Math.sin((f.max-f.life)*8)),0,Math.PI*2);ctx.stroke();ctx.restore();
    }else if(f.kind==="leechField"){
      const owner=s.tanks.find(t=>t.id===f.owner);ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=14;ctx.lineWidth=3;ctx.setLineDash([7,5]);if(owner)for(const id of f.targetIds){const t=s.tanks.find(q=>q.id===id&&q.alive);if(t){ctx.beginPath();ctx.moveTo(t.x,t.y);ctx.quadraticCurveTo((t.x+owner.x)/2,t.y-55,owner.x,owner.y);ctx.stroke();}}ctx.setLineDash([]);ctx.restore();
    }else if(f.kind==="tetrisDrop"){
      ctx.save();ctx.globalAlpha=.3;ctx.strokeStyle=f.color;ctx.setLineDash([5,5]);ctx.strokeRect(f.x-f.span*.55,55,f.span*1.1,Math.max(20,terrainY(s,f.x)-55));ctx.setLineDash([]);ctx.restore();
    }else if(f.kind==="tetrisReactor"){
      ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=22;ctx.lineWidth=8;ctx.globalAlpha=.7;ctx.beginPath();ctx.moveTo(f.scanX??f.x-f.span*.55,40);ctx.lineTo(f.scanX??f.x-f.span*.55,terrainY(s,f.scanX??f.x));ctx.stroke();ctx.restore();
    }else if(f.kind==="eclipseField"){
      ctx.save();for(let m=0;m<f.moons;m++){const cx=f[`moon${m}`]??f.x,cy=74+m*28;ctx.shadowColor="#6879ff";ctx.shadowBlur=18;ctx.fillStyle="#050812";ctx.strokeStyle="#8094ff";ctx.beginPath();ctx.arc(cx,cy,24,0,Math.PI*2);ctx.fill();ctx.stroke();const gy=terrainY(s,clamp(cx,0,s.width-1));const g=ctx.createLinearGradient(cx,cy,cx,gy);g.addColorStop(0,"rgba(50,55,105,.05)");g.addColorStop(1,"rgba(35,35,70,.46)");ctx.fillStyle=g;ctx.beginPath();ctx.moveTo(cx-12,cy+18);ctx.lineTo(cx-f.width*.5,gy);ctx.lineTo(cx+f.width*.5,gy);ctx.lineTo(cx+12,cy+18);ctx.closePath();ctx.fill();}ctx.restore();
    }else if(f.kind==="guillotineSweep"){
      if(f.delay<=0){ctx.save();ctx.strokeStyle="#ff6680";ctx.shadowColor="#ff6680";ctx.shadowBlur=22;ctx.lineWidth=9;ctx.globalAlpha=.75;ctx.beginPath();ctx.moveTo(f.x-f.length/2,f.y);ctx.lineTo(f.x+f.length/2,f.y);ctx.stroke();ctx.restore();}
    }else if(f.kind==="slingConverge"){
      const t=1-f.life/f.max;ctx.save();ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=14;ctx.lineWidth=3;for(const side of [-1,1]){ctx.beginPath();ctx.moveTo(f.x+side*120,terrainY(s,clamp(f.x+side*120,0,s.width-1))-8);ctx.quadraticCurveTo(f.x+side*60,f.y-90,f.x,f.y);ctx.stroke();}ctx.globalAlpha=.5;ctx.beginPath();ctx.arc(f.x,f.y,8+20*t,0,Math.PI*2);ctx.stroke();ctx.restore();
    }else if(f.kind==="gravity"||f.kind==="voidwell"){
...
function drawFx(ctx,s){
  for(const f of s.fx){
    ...
    }else if(f.kind==="sonicBand"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#82f7ff";ctx.shadowColor=f.color||"#82f7ff";ctx.shadowBlur=16;ctx.lineWidth=2.4;const w=f.width||28,dx=f.x2-f.x1,dy=f.y2-f.y1,l=Math.max(1,Math.hypot(dx,dy)),nx=-dy/l,ny=dx/l;for(const q of [-1,1]){ctx.beginPath();ctx.moveTo(f.x1+nx*w*q*.15,f.y1+ny*w*q*.15);ctx.lineTo(f.x2+nx*w*q,f.y2+ny*w*q);ctx.stroke();}ctx.globalAlpha=t*.35;ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(f.x1,f.y1);ctx.lineTo(f.x2,f.y2);ctx.stroke();ctx.restore();
    }else if(f.kind==="sonicCrown"){
      ctx.save();ctx.translate(f.x,f.y);ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#9ff8ff";ctx.shadowColor=f.color||"#9ff8ff";ctx.shadowBlur=22;for(let k=0;k<3;k++){const r=(1-t)*(f.r||72)+15+k*9;ctx.lineWidth=3-k*.5;ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.stroke();for(let i=0;i<12;i++){const a=i/12*Math.PI*2;ctx.beginPath();ctx.moveTo(Math.cos(a)*(r-6),Math.sin(a)*(r-6));ctx.lineTo(Math.cos(a)*(r+8),Math.sin(a)*(r+8));ctx.stroke();}}ctx.restore();
    }else if(["aegisDeploy","aegisReflect","aegisAbsorb"].includes(f.kind)){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#7cecff";ctx.shadowColor=f.color||"#7cecff";ctx.shadowBlur=20;ctx.lineWidth=f.kind==="aegisReflect"?4:2.5;const rr=(f.r||22)*(1+(1-t)*.35);ctx.beginPath();ctx.arc(f.x,f.y,rr,f.kind==="aegisDeploy"?Math.PI:0,f.kind==="aegisDeploy"?Math.PI*2:Math.PI*2);ctx.stroke();if(f.kind==="aegisReflect"){ctx.beginPath();ctx.moveTo(f.x-12,f.y+6);ctx.lineTo(f.x,f.y-10);ctx.lineTo(f.x+12,f.y+6);ctx.stroke();}if(f.kind==="aegisAbsorb"){ctx.fillStyle=f.color;ctx.globalAlpha=t*.25;ctx.beginPath();ctx.arc(f.x,f.y,rr*.75,0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="dozerDirt"||f.kind==="landslideDust"){
      ctx.save();ctx.globalAlpha=t*.75;const span=f.span||35;for(let i=0;i<12;i++){const q=(i/11-.5),x=f.x+q*span+(1-t)*q*14,y=f.y-3-Math.abs(Math.sin(i*2.1))*8-(1-t)*10;ctx.fillStyle=i%2?"#b88b62":"#775846";ctx.beginPath();ctx.arc(x,y,2+(i%3),0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="adaptiveMorph"){
      ctx.save();ctx.translate(f.x,f.y);ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#b7f7ff";ctx.shadowColor=f.color||"#b7f7ff";ctx.shadowBlur=20;ctx.lineWidth=2.5;for(let k=0;k<3;k++){const r=10+k*8+(1-t)*12;ctx.beginPath();for(let i=0;i<6;i++){const a=i/6*Math.PI*2+f.life*2*(k%2?1:-1),x=Math.cos(a)*r,y=Math.sin(a)*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.closePath();ctx.stroke();}ctx.restore();
    }else if(f.kind==="stampImpact"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#f4d27d";ctx.shadowColor=f.color||"#f4d27d";ctx.shadowBlur=18;ctx.lineWidth=3;const span=f.span||100,teeth=f.teeth||3;ctx.beginPath();ctx.moveTo(f.x-span*.5,f.y);ctx.lineTo(f.x+span*.5,f.y);ctx.stroke();for(let i=0;i<teeth;i++){const x=f.x+(i-(teeth-1)/2)*(span*.72/Math.max(1,teeth-1));ctx.beginPath();ctx.moveTo(x,f.y);ctx.lineTo(x,f.y-18-(1-t)*15);ctx.stroke();}ctx.restore();
    }else if(f.kind==="cradlePulse"){
      const pts=f.points||[];ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#fff";ctx.shadowColor=f.color||"#fff";ctx.shadowBlur=14;ctx.lineWidth=3;if(pts.length){ctx.beginPath();for(let i=0;i<pts.length;i++){const q=pts[i];i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);}ctx.stroke();for(const q of pts){ctx.fillStyle="#efffff";ctx.beginPath();ctx.arc(q.x,q.y,4+(1-t)*4,0,Math.PI*2);ctx.fill();}}ctx.restore();
    }else if(f.kind==="beltDeploy"||f.kind==="beltCrush"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#74ffd2";ctx.shadowColor=f.color||"#74ffd2";ctx.shadowBlur=16;ctx.lineWidth=f.kind==="beltCrush"?5:3;if(f.kind==="beltDeploy"){ctx.beginPath();ctx.moveTo(f.x-(f.span||100)/2,f.y);ctx.lineTo(f.x+(f.span||100)/2,f.y);ctx.stroke();}else{for(let i=0;i<4;i++){const a=i/4*Math.PI*2,r=8+(1-t)*25;ctx.beginPath();ctx.moveTo(f.x+Math.cos(a)*4,f.y+Math.sin(a)*4);ctx.lineTo(f.x+Math.cos(a)*r,f.y+Math.sin(a)*r);ctx.stroke();}}ctx.restore();
    }else if(f.kind==="anchorBreak"){
      ctx.save();ctx.translate(f.x,f.y);ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#dbe8ef";ctx.shadowColor=f.color||"#dbe8ef";ctx.shadowBlur=14;ctx.lineWidth=2.5;for(let i=0;i<8;i++){const a=i/8*Math.PI*2,r=10+(1-t)*32;ctx.beginPath();ctx.moveTo(Math.cos(a)*5,Math.sin(a)*5);ctx.lineTo(Math.cos(a)*r,Math.sin(a)*r);ctx.stroke();}ctx.restore();
    }else if(f.kind==="bubblePop"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color||"#8deeff";ctx.shadowColor=f.color||"#8deeff";ctx.shadowBlur=14;for(let i=0;i<9;i++){const a=i/9*Math.PI*2,r=8+(1-t)*34;ctx.beginPath();ctx.arc(f.x+Math.cos(a)*r,f.y+Math.sin(a)*r,2+(i%3),0,Math.PI*2);ctx.stroke();}ctx.restore();
    }else if(f.kind==="electronOrbit"){
      ctx.save();ctx.globalAlpha=t*.95;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(f.cx,f.cy);ctx.lineTo(f.x,f.y);ctx.stroke();ctx.fillStyle="#eaffff";ctx.beginPath();ctx.arc(f.x,f.y,2.8,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="waterBurst"){
      ctx.save();ctx.globalAlpha=t;ctx.shadowColor=f.color;ctx.shadowBlur=14;for(let i=0;i<(f.count||10);i++){const a=(i/(f.count||10))*Math.PI+Math.PI,rr=(1-t)*48+(i%3)*5;ctx.fillStyle=i%2?"#baf5ff":f.color;ctx.beginPath();ctx.arc(f.x+Math.cos(a)*rr,f.y-Math.abs(Math.sin(a))*rr*.52,2.2+(i%2),0,Math.PI*2);ctx.fill();}ctx.restore();
    }else if(f.kind==="imploderBlast"){
      ctx.save();ctx.translate(f.x,f.y);ctx.shadowColor=f.color;ctx.shadowBlur=30;for(let k=0;k<5;k++){ctx.globalAlpha=t*(.82-k*.12);ctx.strokeStyle=k===0?"#fff":f.color;ctx.lineWidth=5-k*.6;ctx.beginPath();ctx.arc(0,0,(f.r||90)*(1-t*.12)+(1-t)*k*16,0,Math.PI*2);ctx.stroke();}ctx.globalAlpha=t*.35;ctx.fillStyle=f.color;ctx.beginPath();ctx.arc(0,0,(f.r||90)*.72,0,Math.PI*2);ctx.fill();ctx.restore();
    }else if(f.kind==="teslaPulse"||f.kind==="prismPulse"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=20;ctx.lineWidth=3;const pts=f.points||[];for(let i=0;i<pts.length-1;i++){ctx.beginPath();ctx.moveTo(pts[i].x,pts[i].y);const mx=(pts[i].x+pts[i+1].x)/2+Math.sin(i*7+f.life*30)*7,my=(pts[i].y+pts[i+1].y)/2+Math.cos(i*5+f.life*25)*7;ctx.lineTo(mx,my);ctx.lineTo(pts[i+1].x,pts[i+1].y);ctx.stroke();}if(f.kind==="prismPulse"&&pts.length>2){ctx.beginPath();ctx.moveTo(pts[pts.length-1].x,pts[pts.length-1].y);ctx.lineTo(pts[0].x,pts[0].y);ctx.stroke();}ctx.restore();

    }else if(["sentryBeam","sentryHeavy","carouselRocket","carouselHeavy","beaconFocus","castleCannon"].includes(f.kind)){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=16;ctx.lineWidth=f.kind==="sentryHeavy"||f.kind==="carouselHeavy"?5:2.5;ctx.beginPath();ctx.moveTo(f.x1,f.y1);ctx.lineTo(f.x2,f.y2);ctx.stroke();ctx.restore();
    }else if(f.kind==="tetrisBlock"){
      const u=1-t,yy=f.y+(f.targetY-f.y)*Math.min(1,u*1.6),size=18;ctx.save();ctx.globalAlpha=Math.min(1,t*1.6);ctx.translate(f.x,yy);ctx.shadowColor=f.color;ctx.shadowBlur=12;ctx.fillStyle=f.bomb?"#ff745f":f.color;ctx.strokeStyle="#eaffff";ctx.lineWidth=2;const shape=f.shape||0;for(const [ox,oy] of [[0,0],[1,0],[shape%2?0:1,1],[shape%3?1:-1,1]]){ctx.fillRect((ox-1)*size*.62,(oy-1)*size*.62,size*.58,size*.58);ctx.strokeRect((ox-1)*size*.62,(oy-1)*size*.62,size*.58,size*.58);}ctx.restore();
    }else if(f.kind==="tetrisClear"||f.kind==="executionSweep"||f.kind==="solarColumn"){
      ctx.save();ctx.globalAlpha=t;ctx.shadowColor=f.color;ctx.shadowBlur=24;ctx.strokeStyle=f.color;ctx.lineWidth=f.kind==="solarColumn"?18:7;if(f.kind==="solarColumn"){ctx.beginPath();ctx.moveTo(f.x,0);ctx.lineTo(f.x,f.y);ctx.stroke();}else{ctx.beginPath();ctx.moveTo(f.x-(f.span||f.length||120)/2,f.y);ctx.lineTo(f.x+(f.span||f.length||120)/2,f.y);ctx.stroke();}ctx.restore();
    }else if(f.kind==="rookCross"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=18;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(f.x-100,f.y);ctx.lineTo(f.x+100,f.y);ctx.moveTo(f.x,f.y-110);ctx.lineTo(f.x,f.y+10);ctx.stroke();ctx.restore();
    }else if(f.kind==="portalOpen"||f.kind==="phantomRift"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=20;ctx.lineWidth=3;if(f.x1!=null){for(const [x,y] of [[f.x1,f.y1],[f.x2,f.y2]])for(let k=0;k<3;k++){ctx.beginPath();ctx.ellipse(x,y,12+k*8,5+k*4,f.life*4+k*.5,0,Math.PI*2);ctx.stroke();}}else{for(let k=0;k<3;k++){ctx.beginPath();ctx.ellipse(f.x,f.y,13+k*8,6+k*3,f.life*4+k*.5,0,Math.PI*2);ctx.stroke();}}ctx.restore();
    }else if(f.kind==="swapRift"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=18;ctx.lineWidth=3;const pts=f.points||[];if(pts.length>1){ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);for(let i=1;i<pts.length;i++)ctx.lineTo(pts[i].x,pts[i].y);ctx.lineTo(pts[0].x,pts[0].y);ctx.stroke();}for(const q of pts){ctx.beginPath();ctx.arc(q.x,q.y,10+(1-t)*16,0,Math.PI*2);ctx.stroke();}ctx.restore();
    }else if(f.kind==="crystalGrow"||f.kind==="crystalShatter"||f.kind==="guillotineImpact"||f.kind==="dualSplit"||f.kind==="spreaderCall"||f.kind==="moleculeBurst"){
      ctx.save();ctx.globalAlpha=t;ctx.strokeStyle=f.color;ctx.fillStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=18;if(f.kind==="guillotineImpact"){ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(f.x-(f.length||100)/2,f.y);ctx.lineTo(f.x+(f.length||100)/2,f.y);ctx.stroke();}else{const n=f.count||10,rad=12+(1-t)*42;for(let i=0;i<n;i++){const a=i/n*Math.PI*2;ctx.beginPath();ctx.moveTo(f.x+Math.cos(a)*rad*.45,f.y+Math.sin(a)*rad*.45);ctx.lineTo(f.x+Math.cos(a)*rad,f.y+Math.sin(a)*rad);ctx.stroke();}}ctx.restore();
    ...
    }else if(["spark","edgeBounce","waterDrop","balloonBounce","sandDive","sandPop","pendulumHit","dominoPop","hookMiss","hookSlam","pinballHit","electronHit","repulseRing"].includes(f.kind)){
      ctx.save();ctx.fillStyle=f.color||"#fff";ctx.strokeStyle=f.color||"#fff";ctx.shadowColor=f.color||"#fff";ctx.shadowBlur=f.kind==="repulseRing"?16:9;ctx.globalAlpha=t;if(f.kind==="repulseRing"){ctx.lineWidth=3;ctx.beginPath();ctx.arc(f.x,f.y,(f.r||24)*(1.1-t*.25),0,Math.PI*2);ctx.stroke();}else{const r=f.kind==="waterDrop"?3:f.kind==="sandDive"||f.kind==="sandPop"?3.5:5;ctx.beginPath();ctx.arc(f.x,f.y,r,0,Math.PI*2);ctx.fill();if(f.kind==="balloonBounce"||f.kind==="pendulumHit"){ctx.globalAlpha=t*.45;ctx.beginPath();ctx.arc(f.x,f.y,r+8*(1-t),0,Math.PI*2);ctx.stroke();}}ctx.restore();
...
function drawAim(ctx,s,W,H){
  ...
  if(t.selected==="imploder"){
    ctx.globalAlpha=.42;ctx.strokeStyle=d.color;ctx.fillStyle=d.color+"18";ctx.lineWidth=2.3;ctx.setLineDash([7,6]);ctx.beginPath();ctx.arc(t.x,t.y-4,d.radius||92,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.setLineDash([]);ctx.fillStyle="#f1d9ff";ctx.font="900 9px Inter,system-ui,sans-serif";ctx.textAlign="center";ctx.fillText("SELF-DETONATE",t.x,t.y-(d.radius||92)-12);ctx.restore();return;
  }
...
function drawHud(ctx,s,W,H){
  const t=currentTank(s),compact=W<1080||H<650,pad=compact?20:28,top=12,barH=compact?58:64;
  ctx.save();
  const g=ctx.createLinearGradient(14,top,W-14,top);g.addColorStop(0,"rgba(6,14,23,.90)");g.addColorStop(.5,"rgba(8,17,28,.86)");g.addColorStop(1,"rgba(10,13,25,.90)");
  ctx.fillStyle=g;rr(ctx,14,top,W-28,barH,12);ctx.fill();ctx.strokeStyle="rgba(103,224,255,.16)";ctx.lineWidth=1;ctx.stroke();

  ctx.textAlign="left";ctx.fillStyle="#f1f8fa";ctx.font=`1000 ${compact?13:15}px Inter,system-ui,sans-serif`;ctx.fillText(`ROUND ${s.round}`,pad,top+24);
  ctx.font=`800 ${compact?9:10}px Inter,system-ui,sans-serif`;ctx.fillStyle="#778d9a";ctx.fillText(`${s.arena.name}${s.rogueRun?` · ROGUE ${s.rogueRun.stage}`:""}${s.training?" · TRAINING":""}`,pad,top+44);

  const wd=t?getWeaponTierStats(t.selected,t.selectedTier||1):null;
  ctx.textAlign="center";ctx.fillStyle=t?.color||"#fff";ctx.font=`1000 ${compact?13:15}px Inter,system-ui,sans-serif`;ctx.fillText(t?`${t.name}`:"—",W/2,top+23);
  ctx.font=`800 ${compact?9:10}px Inter,system-ui,sans-serif`;ctx.fillStyle=wd?.color||"#8ca1ad";ctx.fillText(t?`${wd?.name||"—"} · T${t.selectedTier||1}`:"—",W/2,top+43);
  const timerText=s.training?"∞":`${Math.ceil(s.turnTimer)}s`;const tw=compact?46:52;ctx.fillStyle=s.turnTimer<=6&&!s.training?"rgba(255,83,105,.20)":"rgba(93,218,255,.11)";rr(ctx,W/2-tw/2,top+barH-13,tw,21,7);ctx.fill();ctx.strokeStyle=s.turnTimer<=6&&!s.training?"rgba(255,93,112,.55)":"rgba(105,226,255,.28)";ctx.stroke();ctx.fillStyle=s.turnTimer<=6&&!s.training?"#ff8b99":"#bfefff";ctx.font=`1000 ${compact?9:10}px Inter,system-ui,sans-serif`;ctx.fillText(timerText,W/2,top+barH+2);

  ctx.textAlign="right";ctx.font=`1000 ${compact?12:14}px Inter,system-ui,sans-serif`;ctx.fillStyle=s.wind>=0?"#75dbff":"#c38dff";ctx.fillText(`${s.wind>=0?"→":"←"} ${Math.abs(s.wind).toFixed(0)} WIND`,W-pad,top+25);
  ctx.font=`800 ${compact?8:9}px Inter,system-ui,sans-serif`;ctx.fillStyle="#778d9a";const secondary=s.training?`${s.tanks.length-1} DUMMIES · R RESET`:`RESTOCK ${s.playerShotsFired%8}/8`;ctx.fillText(secondary,W-pad,top+44);

  let modeText="",modeColor="#74e6ff";
  if(s.mode==="assassin"){const p=s.tanks[0],target=getAssassinTarget(s,p),hunter=getAssassinHunter(s,p);modeText=`⌖ TARGET ${target?.name||"—"} · HUNTER ${hunter?.name||"—"}`;modeColor="#d998ff";}
  else if(s.mode==="juggernaut"){const jug=s.tanks.find(q=>q.isJuggernaut),hunters=s.tanks.filter(q=>q.alive&&!q.isJuggernaut).length;modeText=jug?.alive?`♛ ${Math.ceil(jug.hp)}/${jug.maxHp} HP · ${hunters} HUNTERS LEFT`:"♛ JUGGERNAUT DOWN";modeColor="#ffc45e";}
  if(modeText){ctx.textAlign="center";ctx.font=`1000 ${compact?9:10}px Inter,system-ui,sans-serif`;const mw=ctx.measureText(modeText).width+24;ctx.fillStyle="rgba(6,13,22,.86)";rr(ctx,W/2-mw/2,top+barH+16,mw,25,8);ctx.fill();ctx.strokeStyle=modeColor+"88";ctx.stroke();ctx.fillStyle=modeColor;ctx.fillText(modeText,W/2,top+barH+33);}
  if(s.messageTimer>0){ctx.textAlign="center";ctx.font=`1000 ${compact?15:17}px Inter,system-ui,sans-serif`;ctx.fillStyle="#fff";ctx.shadowColor="rgba(85,220,255,.3)";ctx.shadowBlur=10;ctx.fillText(s.message,W/2,top+barH+(modeText?66:37));}
  ctx.restore();
}
