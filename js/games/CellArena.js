...
import { createWorld, center, moveOwner, sameOwnerPhysics, splitOwner, ejectOwner, updateWorld, ranking, getModeStatus, playerScore, fmtTime, clamp, teamMasses } from './CellArenaEngine.js';
...
export default {
  ...
  init: (container, services) => {
    ...
    style.textContent=`
      ...
      .ca-map{position:absolute;right:12px;bottom:12px;width:155px;height:155px;background:#ffffffd2;border:1px solid #0003}.ca-map canvas{width:100%;height:100%}.ca-audio,.ca-theme{position:absolute;z-index:12;top:12px;padding:7px 9px;background:#ffffffdc;border:1px solid #0002;color:#666;font:inherit;font-size:.62rem;cursor:pointer}.ca-audio{left:12px}.ca-theme{left:86px}
    ...
    root.innerHTML=`
      ...
      <div class="ca-ov ca-menu"><div class="ca-card">
        ...
        <div class="ca-rules"><div class="ca-rule"><b>Vanilla-like physics</b>Split boost, 30s+ mass-based merge delay, rigid cell collision, ~30% mass advantage to eat and gradual decay.</div><div class="ca-rule"><b>Advanced bots</b>Rookies remain imperfect; Veterans/Aces predict split reach, chain split and use Viruses more deliberately.</div><div class="ca-rule"><b>FFA duos</b>Some paired bots regroup, cover each other, feed mass, bait enemies and punish attacks together.</div></div>
      </div></div>
    ...
    function reset(){
      world=createWorld(selectedMode,{playerName,playerColor,settings:gameSettings});camera={x:world.size/2,y:world.size/2,zoom:CONFIG.baseZoom,targetZoom:CONFIG.baseZoom};running=true;ended=false;lastPlayerAlive=true;end.classList.add('hide');leaderboard();updateHud();
    }
    ...
    function finish(reason=''){
      ...
      const title=$('.ca-end-title'),sub=$('.ca-end-sub');if(world?.mode?.battle&&world?.winner?.isPlayer){title.textContent='Battle Royale Victory!';title.style.color='#d6ac43';}else if(world?.finished&&world?.winner?.isPlayer){title.textContent='You won!';title.style.color='#5dbd71';}else{title.textContent=world?.player?.alive?'Match complete':'You were eaten!';title.style.color='#ec5c68';}sub.textContent=reason||world?.finishReason||`${world?.mode?.name||'Arena'} round finished.`;end.classList.remove('hide');tone(world?.winner?.isPlayer?620:155,.10,.016);
    }
    function update(dt){
      if(!running||!world)return;updatePlayer(dt);for(const o of world.bots)updateBotAI(world,o,dt);updateWorld(world,dt);updateCamera();updateHud();const alive=!!world.player?.alive&&world.player.cells.length>0;if(lastPlayerAlive&&!alive){lastPlayerAlive=false;finish(world.mode.battle?'Eliminated from Battle Royale.':'Your cells were consumed.');return;}if(world.finished)finish(world.finishReason);
    }
